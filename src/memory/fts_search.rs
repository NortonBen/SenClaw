...
impl Default for SearchOptions {
    fn default() -> Self {
        Self { max_results: 6, min_score: 0.25, source: None }
    }
}
    ...
    if let Some(provider) = embedding_provider {
        match mixed_search(db, folder, query, provider, source_filter, max_results).await {
            Ok(results) if !results.is_empty() => {
                return Ok(results.into_iter().filter(|r| r.score >= min_score).take(max_results).collect());
            }
            Err(e) => {
...
fn fts_search(
    ...
    let fts_query = expanded.iter().map(|t| sanitize(t)).filter(|t| !t.is_empty()).collect::<Vec<_>>().join(" OR ");
    ...
    let rows: Vec<FtsRow> = if source_filter != "all" {
        ...
        let mapped = stmt.query_map(params![fts_query, folder, source_filter, limit as i64], row_to_fts)?;
    ...
    Ok(rows.into_iter().map(|r| SearchResult {
        id: r.id, path: r.path, start_line: r.start_line, end_line: r.end_line,
        text: r.text, source: r.source,
        score: if range == 0.0 { 1.0 } else { ((max_rank - r.rank) / range) as f32 },
    }).collect())
}

fn row_to_fts(row: &rusqlite::Row<'_>) -> rusqlite::Result<FtsRow> {
    Ok(FtsRow {
        id: row.get(0)?, path: row.get(1)?, start_line: row.get(2)?,
        end_line: row.get(3)?, text: row.get(4)?, source: row.get(5)?, rank: row.get(6)?,
    })
}

struct FtsRow { id: String, path: String, start_line: u32, end_line: u32, text: String, source: String, rank: f64 }

// ===== Keyword substring fallback =====

fn keyword_fallback(
    conn: &Connection, folder: &str, query: &str, source_filter: &str, limit: usize,
) -> Result<Vec<SearchResult>> {
    let rewritten = smart_rewrite_query(query);
    let tokens = tokenize_optimized(&rewritten, true);
    let ngrams = generate_2gram(&rewritten);
    let all_tokens: Vec<String> = tokens.into_iter().chain(ngrams).collect();
    if all_tokens.is_empty() { return Ok(vec![]); }
    ...
    for row in &rows {
        ...
        let row_tokens: std::collections::HashSet<String> =
            tokenize_optimized(&row.text, false).into_iter().chain(generate_2gram(&row.text)).collect();
        let match_count = all_tokens.iter()
            .filter(|t| row_tokens.contains(t.as_str()) || text_lower.contains(&t.to_lowercase()))
            .count();
        if match_count > 0 {
            results.push(SearchResult {
                id: row.id.clone(), path: row.path.clone(),
                start_line: row.start_line, end_line: row.end_line,
                text: row.text.clone(), source: row.source.clone(),
...
fn row_to_chunk(row: &rusqlite::Row<'_>) -> rusqlite::Result<ChunkRow> {
    Ok(ChunkRow {
        id: row.get(0)?, path: row.get(1)?, start_line: row.get(2)?,
        end_line: row.get(3)?, text: row.get(4)?, source: row.get(5)?,
    })
}

struct ChunkRow { id: String, path: String, start_line: u32, end_line: u32, text: String, source: String }

// ===== Mixed/hybrid search =====

async fn mixed_search(
    db: &Db, folder: &str, query: &str, provider: &dyn EmbeddingProvider,
    source_filter: &str, limit: usize,
) -> Result<Vec<SearchResult>> {
    let emb = provider.embed(&[query.to_string()]).await.context("embed query")?;
    ...
    if vec_results.is_empty() { return Ok(vec![]); }
...
fn vec_search(
    conn: &Connection, folder: &str, query_embedding: &[f32],
    source_filter: &str, limit: usize,
) -> Result<Vec<SearchResult>> {
...
fn try_vec0(
    ...
    let query_buf: Vec<u8> = query_embedding.iter().flat_map(|f| f.to_le_bytes()).collect();
    let total: i64 = conn.query_row("SELECT COUNT(*) FROM memory_chunks_vec", [], |r| r.get(0)).unwrap_or(0);
    ...
    let rows: Vec<VecDistanceRow> = if source_filter != "all" {
        ...
        let mapped = stmt.query_map(params![query_buf, k, folder, source_filter], row_to_vec_dist)?;
        mapped.collect::<rusqlite::Result<Vec<_>>>()?
    } else {
        let mut stmt = conn.prepare(
            "SELECT v.chunk_id, c.path, c.start_line, c.end_line, c.text, c.source, v.distance \
...
fn row_to_vec_dist(row: &rusqlite::Row<'_>) -> rusqlite::Result<VecDistanceRow> {
    Ok(VecDistanceRow {
        id: row.get(0)?, path: row.get(1)?, start_line: row.get(2)?,
        end_line: row.get(3)?, text: row.get(4)?, source: row.get(5)?, distance: row.get(6)?,
    })
}
...
fn try_blob_scan(
    ...
    let mut with_dist: Vec<VecDistanceRow> = rows.into_iter().filter_map(|row| {
        let emb = row.embedding?;
        let dist = cosine_distance(query_embedding, &emb)?;
        Some(VecDistanceRow {
            id: row.id, path: row.path, start_line: row.start_line,
            end_line: row.end_line, text: row.text, source: row.source, distance: dist,
        })
    }).collect();

    with_dist.sort_by(|a, b| a.distance.partial_cmp(&b.distance).unwrap_or(std::cmp::Ordering::Equal));
...
fn row_to_chunk_emb(row: &rusqlite::Row<'_>) -> rusqlite::Result<ChunkEmbRow> {
    Ok(ChunkEmbRow {
        id: row.get(0)?, path: row.get(1)?, start_line: row.get(2)?,
        end_line: row.get(3)?, text: row.get(4)?, source: row.get(5)?, embedding: row.get(6)?,
    })
}

fn cosine_distance(a: &[f32], b_blob: &[u8]) -> Option<f32> {
    if b_blob.len() % 4 != 0 { return None; }
    let b: Vec<f32> = b_blob.chunks_exact(4).map(|c| f32::from_le_bytes([c[0], c[1], c[2], c[3]])).collect();
    if a.len() != b.len() { return None; }
    let dot: f32 = a.iter().zip(b.iter()).map(|(x, y)| x * y).sum();
    let na: f32 = a.iter().map(|x| x * x).sum::<f32>().sqrt();
    let nb: f32 = b.iter().map(|x| x * x).sum::<f32>().sqrt();
    if na == 0.0 || nb == 0.0 { return Some(1.0); }
    Some(1.0 - dot / (na * nb))
}
...
fn normalize_vec_results(rows: Vec<VecDistanceRow>) -> Vec<SearchResult> {
    ...
    if dist_range < 0.05 || min_dist > 0.6 { return vec![]; }
    rows.into_iter().map(|r| SearchResult {
        id: r.id, path: r.path, start_line: r.start_line, end_line: r.end_line,
        text: r.text, source: r.source,
...
struct VecDistanceRow { id: String, path: String, start_line: u32, end_line: u32, text: String, source: String, distance: f32 }
struct ChunkEmbRow { id: String, path: String, start_line: u32, end_line: u32, text: String, source: String, embedding: Option<Vec<u8>> }
...
mod tests {
    ...
    fn cosine_distance_orthogonal() {
        let a = vec![1.0, 0.0];
        let b: Vec<u8> = vec![0.0f32, 1.0f32].iter().flat_map(|f| f.to_le_bytes()).collect();
        let d = cosine_distance(&a, &b).unwrap();
