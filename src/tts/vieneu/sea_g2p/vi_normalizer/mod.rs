...
use crate::tts::vieneu::sea_g2p::vi_normalizer::numerical::{normalize_number_vi, RE_MULTIPLY, expand_multiply_number};
use crate::tts::vieneu::sea_g2p::vi_normalizer::datestime::{normalize_date, normalize_time};
use crate::tts::vieneu::sea_g2p::vi_normalizer::units::{expand_units_and_currency, expand_compound_units, expand_scientific_notation, fix_english_style_numbers, expand_power_of_ten, expand_height_weight};
use crate::tts::vieneu::sea_g2p::vi_normalizer::misc::{normalize_others, expand_standalone_letters, RE_ACRONYMS_EXCEPTIONS, RE_ACRONYM};
use crate::tts::vieneu::sea_g2p::vi_normalizer::technical::{normalize_technical, normalize_emails, RE_TECHNICAL, RE_EMAIL};
use crate::tts::vieneu::sea_g2p::vi_normalizer::resources::{COMBINED_EXCEPTIONS, MEASUREMENT_KEY_VI};
...
fn expand_money_slang(text: &str) -> String {
    let res = RE_MONEY_TR.replace_all(text, |caps: &FCaps| {
        let x = crate::tts::vieneu::sea_g2p::vi_normalizer::num2vi::n2w(caps.get(1).unwrap().as_str());
        match caps.get(2) {
            Some(y) => format!(" {} triệu {} trăm nghìn ", x, crate::tts::vieneu::sea_g2p::vi_normalizer::num2vi::n2w(y.as_str())),
            None => format!(" {} triệu ", x),
        }
    }).into_owned();
    RE_MONEY_K.replace_all(&res, |caps: &FCaps| {
        format!(" {} nghìn ", crate::tts::vieneu::sea_g2p::vi_normalizer::num2vi::n2w(caps.get(1).unwrap().as_str()))
    }).into_owned()
}
...
pub fn clean_vietnamese_text(text: &str) -> String {
    ...
    current_text = crate::tts::vieneu::sea_g2p::vi_normalizer::misc::expand_abbreviations(&current_text);
    ...
    current_text = RE_HOTLINE.replace_all(&current_text, |caps: &Captures| {
        format!(" {} {} ",
            crate::tts::vieneu::sea_g2p::vi_normalizer::num2vi::n2w_single(caps.get(1).unwrap().as_str()),
            crate::tts::vieneu::sea_g2p::vi_normalizer::num2vi::n2w_single(caps.get(2).unwrap().as_str())
        )
    }).into_owned();
    ...
    current_text = RE_POWER_OF_TEN_IMPLICIT.replace_all(&current_text, |caps: &Captures| {
        ...
        if exp.starts_with('-') {
            format!("mười mũ trừ {}", crate::tts::vieneu::sea_g2p::vi_normalizer::num2vi::n2w(&exp[1..]))
        } else {
            format!("mười mũ {}", crate::tts::vieneu::sea_g2p::vi_normalizer::num2vi::n2w(&exp.replace('+', "")))
        }
    }).into_owned();
    ...
    current_text = crate::tts::vieneu::sea_g2p::vi_normalizer::misc::expand_size_labels(&current_text);
    ...
    current_text = RE_LONG_NUM.replace_all(&current_text, |caps: &FCaps| {
        ...
        format!(" {}{} ", neg_prefix, crate::tts::vieneu::sea_g2p::vi_normalizer::num2vi::n2w_single(num_str))
    }).to_string();

    current_text = fix_english_style_numbers(&current_text);

    current_text = RE_MULTI_COMMA.replace_all(&current_text, |caps: &Captures| {
        caps.get(1).unwrap().as_str().split(',').map(|s: &str| crate::tts::vieneu::sea_g2p::vi_normalizer::num2vi::n2w_decimal(s)).collect::<Vec<String>>().join(" phẩy ")
    }).into_owned();

    current_text = RE_FLOAT_WITH_COMMA.replace_all(&current_text, |caps: &FCaps| {
        let int_part = crate::tts::vieneu::sea_g2p::vi_normalizer::num2vi::n2w(&caps.get(1).unwrap().as_str().replace('.', ""));
        let dec_part = caps.get(2).unwrap().as_str().trim_end_matches('0');
        let mut res = if dec_part.is_empty() { int_part } else { format!("{} phẩy {}", int_part, crate::tts::vieneu::sea_g2p::vi_normalizer::num2vi::n2w_decimal(dec_part)) };
...
impl Normalizer {
    ...
    pub fn normalize_batch(&self, texts: Vec<String>, punc_norm: bool) -> Vec<String> {
        texts.into_iter().map(|t| self.normalize(&t, punc_norm)).collect()
    }
}
