...
use crate::tts::vieneu::sea_g2p::vi_normalizer::datestime::{normalize_date, normalize_time};
use crate::tts::vieneu::sea_g2p::vi_normalizer::misc::{
    expand_standalone_letters, normalize_others, RE_ACRONYM, RE_ACRONYMS_EXCEPTIONS,
};
use crate::tts::vieneu::sea_g2p::vi_normalizer::numerical::{
    expand_multiply_number, normalize_number_vi, RE_MULTIPLY,
};
use crate::tts::vieneu::sea_g2p::vi_normalizer::resources::{
    COMBINED_EXCEPTIONS, MEASUREMENT_KEY_VI,
};
use crate::tts::vieneu::sea_g2p::vi_normalizer::technical::{
    normalize_emails, normalize_technical, RE_EMAIL, RE_TECHNICAL,
};
use crate::tts::vieneu::sea_g2p::vi_normalizer::units::{
...
static RE_RANGE: Lazy<FRegex> = Lazy::new(|| {
    FRegex::new(r"(?<!\d)(?<!\d[,.])(?<![a-zA-Z])(\d{1,15}(?:[,.]\d{1,15})?)(\s*)[\u2013\-\u2014](\s*)(\d{1,15}(?:[,.]\d{1,15})?)(?!\d)(?![.,]\d)").unwrap()
});
...
static RE_NEG_VAR1: Lazy<FRegex> = Lazy::new(|| {
    FRegex::new(r"(?<=[(\[{=\u00b1+*/\u00d7\u00f7])[-\u2013\u2014]([a-zA-Z])").unwrap()
});
static RE_NEG_VAR2: Lazy<FRegex> = Lazy::new(|| {
    FRegex::new(r"(?<=[(\[{=\u00b1+*/\u00d7\u00f7]\s)[-\u2013\u2014]([a-zA-Z])").unwrap()
});
...
fn expand_scores(text: &str) -> String {
    use crate::tts::vieneu::sea_g2p::vi_normalizer::num2vi::n2w;
    let res = RE_SCORE_KW
...
fn expand_money_slang(text: &str) -> String {
    let res = RE_MONEY_TR
        .replace_all(text, |caps: &FCaps| {
            let x = crate::tts::vieneu::sea_g2p::vi_normalizer::num2vi::n2w(
                caps.get(1).unwrap().as_str(),
            );
            match caps.get(2) {
                Some(y) => format!(
                    " {} triệu {} trăm nghìn ",
                    x,
                    crate::tts::vieneu::sea_g2p::vi_normalizer::num2vi::n2w(y.as_str())
                ),
                None => format!(" {} triệu ", x),
            }
        })
    ...
    RE_MONEY_K
        .replace_all(&res, |caps: &FCaps| {
            format!(
                " {} nghìn ",
                crate::tts::vieneu::sea_g2p::vi_normalizer::num2vi::n2w(
                    caps.get(1).unwrap().as_str()
                )
            )
        })
...
pub fn clean_vietnamese_text(text: &str) -> String {
    ...
    current_text = RE_MULTIPLY
        .replace_all(&current_text, |caps: &FCaps| {
            expand_multiply_number(caps.get(0).unwrap().as_str())
        })
        .to_string();

    current_text = RE_CONTEXT_TRU
        .replace_all(&current_text, " $1 $2 trừ $3 ")
        .into_owned();
    current_text = RE_CONTEXT_TRU_POST
        .replace_all(&current_text, " $1 trừ $2 $3 ")
        .into_owned();
    current_text = RE_CONTEXT_DEN
        .replace_all(&current_text, " $1 $2 đến $3 ")
        .into_owned();

    current_text = RE_EQ_MINUS
        .replace_all(&current_text, |caps: &Captures| {
            format!(
                "{} trừ {} =",
                caps.get(1).unwrap().as_str(),
    ...
    current_text =
        crate::tts::vieneu::sea_g2p::vi_normalizer::misc::expand_abbreviations(&current_text);
    current_text = RE_ADDR_ABBR
        .replace_all(&current_text, |caps: &FCaps| {
    ...
    current_text = RE_RANGE_PCT
        .replace_all(&current_text, "$1 đến $2%")
        .into_owned();
    current_text = RE_PCT_NEG
        .replace_all(&current_text, " âm $1% ")
        .into_owned();
    ...
    current_text = RE_NUMERIC_DASH_GROUPS
        .replace_all(&current_text, |caps: &Captures| {
            let matched = caps.get(0).unwrap().as_str();
            let parts: Vec<&str> = matched.split(&['-', '\u{2013}', '\u{2014}'][..]).collect();
            parts
                .iter()
                .map(|&p| crate::tts::vieneu::sea_g2p::vi_normalizer::num2vi::n2w_single(p))
                .collect::<Vec<String>>()
                .join(", ")
        })
        .into_owned();

    current_text = RE_LANDLINE
        .replace_all(&current_text, |caps: &FCaps| {
            let matched = caps.get(0).unwrap().as_str();
            let groups: Vec<String> = matched
                .split(|c: char| !c.is_ascii_digit())
                .filter(|s: &&str| !s.is_empty())
                .map(|s: &str| crate::tts::vieneu::sea_g2p::vi_normalizer::num2vi::n2w_single(s))
                .collect();
    ...
    current_text = RE_PHONE_WITH_DASH
        .replace_all(&current_text, |caps: &Captures| {
            ...
            format!(
                " {}, {}, {} ",
                crate::tts::vieneu::sea_g2p::vi_normalizer::num2vi::n2w_single(p1),
                crate::tts::vieneu::sea_g2p::vi_normalizer::num2vi::n2w_single(p2),
                crate::tts::vieneu::sea_g2p::vi_normalizer::num2vi::n2w_single(p3)
            )
        })
        .into_owned();

    current_text = RE_HOTLINE
        .replace_all(&current_text, |caps: &Captures| {
            format!(
                " {} {} ",
                crate::tts::vieneu::sea_g2p::vi_normalizer::num2vi::n2w_single(
                    caps.get(1).unwrap().as_str()
                ),
                crate::tts::vieneu::sea_g2p::vi_normalizer::num2vi::n2w_single(
                    caps.get(2).unwrap().as_str()
    ...
    current_text = RE_POWER_OF_TEN_IMPLICIT
        .replace_all(&current_text, |caps: &Captures| {
            let exp = caps.get(1).unwrap().as_str();
            if exp.starts_with('-') {
                format!(
                    "mười mũ trừ {}",
                    crate::tts::vieneu::sea_g2p::vi_normalizer::num2vi::n2w(&exp[1..])
                )
            } else {
                format!(
                    "mười mũ {}",
                    crate::tts::vieneu::sea_g2p::vi_normalizer::num2vi::n2w(&exp.replace('+', ""))
                )
            }
        })
        .into_owned();

    current_text = RE_RANGE
        .replace_all(&current_text, |caps: &FCaps| {
    ...
    current_text = RE_NEG_VAR1
        .replace_all(&current_text, " âm $1 ")
        .into_owned();
    current_text = RE_NEG_VAR2
        .replace_all(&current_text, " âm $1 ")
        .into_owned();
    current_text = RE_MATH_MINUS_SUP
        .replace_all(&current_text, " trừ ")
        .into_owned();
    current_text = RE_MATH_MINUS_COEF
        .replace_all(&current_text, " trừ ")
        .into_owned();
    current_text = RE_MATH_MINUS_COEFL
        .replace_all(&current_text, " trừ ")
        .into_owned();
    current_text = RE_DASH_TO_COMMA
    ...
    current_text =
        crate::tts::vieneu::sea_g2p::vi_normalizer::misc::expand_size_labels(&current_text);
    current_text = expand_compound_units(&current_text);
    current_text = expand_units_and_currency(&current_text);
    current_text = RE_LONG_NUM
        .replace_all(&current_text, |caps: &FCaps| {
            ...
            format!(
                " {}{} ",
                neg_prefix,
                crate::tts::vieneu::sea_g2p::vi_normalizer::num2vi::n2w_single(num_str)
            )
        })
        .to_string();
    ...
    current_text = RE_MULTI_COMMA
        .replace_all(&current_text, |caps: &Captures| {
            caps.get(1)
                .unwrap()
                .as_str()
                .split(',')
                .map(|s: &str| crate::tts::vieneu::sea_g2p::vi_normalizer::num2vi::n2w_decimal(s))
                .collect::<Vec<String>>()
                .join(" phẩy ")
        })
        .into_owned();

    current_text = RE_FLOAT_WITH_COMMA
        .replace_all(&current_text, |caps: &FCaps| {
            let int_part = crate::tts::vieneu::sea_g2p::vi_normalizer::num2vi::n2w(
            ...
            let mut res = if dec_part.is_empty() {
                ...
                format!(
                    "{} phẩy {}",
                    int_part,
                    crate::tts::vieneu::sea_g2p::vi_normalizer::num2vi::n2w_decimal(dec_part)
                )
            };
...
impl Normalizer {
    ...
    pub fn normalize(&self, text: &str, punc_norm: bool) -> String {
        ...
        if punc_norm {
            crate::tts::vieneu::sea_g2p::punc::apply_punc_norm(&result)
        } else {
            result
        }
    }
