...
static RE_CURRENCY_PREFIX_SYMBOL: Lazy<Regex> =
    Lazy::new(|| Regex::new(&format!(r"(?i)([$€¥£₩])\s*{}{}", NUMERIC_P, MAGNITUDE_P)).unwrap());
...
static RE_HEIGHT: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?<![\d.,a-zA-Z])(\d{1,2})m(\d{2}|[01456789])(?![\d.,])").unwrap());

// C\u00e2n n\u1eb7ng ki\u1ec3u Vi\u1ec7t: "1kg2" -> "m\u1ed9t ki l\u00f4 gam hai" (2 = l\u1ea1ng/hectogram).
static RE_WEIGHT: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?<![\d.,a-zA-Z])(\d{1,2})kg(\d{1,2})(?![\d.,])").unwrap());
