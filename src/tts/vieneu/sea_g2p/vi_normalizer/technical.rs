use crate::tts::vieneu::sea_g2p::vi_normalizer::num2vi::{n2w, n2w_single};
use crate::tts::vieneu::sea_g2p::vi_normalizer::resources::{
    COMMON_EMAIL_DOMAINS, DOMAIN_SUFFIX_MAP, VI_LETTER_NAMES,
};
                ...
                match s {
                    ...
                    _ => {
                        ...
                        if s.chars().any(|c: char| c.is_alphabetic() && !c.is_ascii()) {
                            ...
                            DOMAIN_SUFFIX_MAP.get(s.to_lowercase().as_str())
                        {
                                ...
                                if sub_tokens.len() > 1 {
                                    for t in sub_tokens {
                                        if t.chars().all(|c: char| c.is_ascii_digit()) {
                                            ...
                                            if (t.chars().all(|c: char| c.is_uppercase())
                                                && t.len() <= 4)
                                                || t.len() <= 2
                                            {
                                                val = val
                                    ...
                                    if (s.chars().all(|c: char| c.is_uppercase()) && s.len() <= 4)
                                        || s.len() <= 2
                                    {
                                        val = val
                                            .chars()
...
pub fn normalize_emails(text: &str) -> String {
    RE_EMAIL
        .replace_all(text, |caps: &Captures| {
                    ...
                    if sub_tokens.len() > 1 {
                        ...
                        for t in sub_tokens {
                            if t.chars().all(|c: char| c.is_ascii_digit()) {
                                ...
                                res_parts
                                    .push(format!("__start_en__{}__end_en__", t.to_lowercase()));
                            }
                        }
            ...
            format!("{} a còng {}", user_norm, domain_norm)
                .replace("  ", " ")
                .trim()
                .to_string()
        })
