use crate::tts::vieneu::sea_g2p::vi_normalizer::num2vi::{n2w, n2w_decimal};
use crate::tts::vieneu::sea_g2p::vi_normalizer::resources::{DATE_KEYWORDS, MATH_KEYWORDS};
...
static RE_QUY_YEAR: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\b(quý)\s+(\d{1,2})\s*/\s*(\d{4})\b").unwrap());
...
static RE_REDUNDANT_NGAY: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)\bngày\s+ngày\b").unwrap());

static RE_REDUNDANT_THANG: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\btháng\s+tháng\b").unwrap());

static RE_REDUNDANT_NAM: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)\bnăm\s+năm\b").unwrap());

static RE_REDUNDANT_HOM_NGAY: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\bhôm\s+ngày\b").unwrap());

static RE_REDUNDANT_MUNG_NGAY: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\b(mùng|mồng)\s+ngày\b").unwrap());
...
pub fn normalize_date(text: &str) -> String {
            ...
            let prefix = if code_lower.starts_with('q') && code.len() <= 2 {
                ...
                for c in code.chars() {
                    ...
                    if c.is_ascii_digit() {
                        ...
                        crate::tts::vieneu::sea_g2p::vi_normalizer::resources::VI_LETTER_NAMES
                            .get(cl.as_str())
                    {
    ...
    result = RE_REDUNDANT_THANG
        .replace_all(&result, "tháng")
        .into_owned();
    result = RE_REDUNDANT_NAM.replace_all(&result, "năm").into_owned();
    result = RE_REDUNDANT_HOM_NGAY
        .replace_all(&result, "hôm")
        .into_owned();
