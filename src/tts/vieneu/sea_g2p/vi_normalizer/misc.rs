...
use crate::tts::vieneu::sea_g2p::vi_normalizer::resources::{
    ...
    SYMBOLS_MAP, VI_LETTER_NAMES, WORD_LIKE_ACRONYMS,
};
use crate::tts::vieneu::sea_g2p::vi_normalizer::technical::normalize_slashes;
...
static RE_STANDALONE_LETTER: Lazy<FRegex> =
    Lazy::new(|| FRegex::new(r"(?<![\''])\b([a-zA-Z])\b(\.?)").unwrap());
pub static RE_ACRONYM: Lazy<FRegex> = Lazy::new(|| {
    FRegex::new(&format!(
        r"\b(?=[A-Z{}a-z{}0-9]*[A-Z{}])(?:[A-Z{}][a-z{}]?\d*){{2,}}\b",
        VI_UPPER, VI_UPPER, VI_UPPER, VI_UPPER, "đăâêôơư"
    ))
    .unwrap()
});
...
pub fn normalize_others(text: &str) -> String {
    ...
    let text = RE_ABS
        ...
        .into_owned();
    let mut res = RE_ACRONYMS_EXCEPTIONS
        .replace_all(&text, |caps: &Captures| {
            COMBINED_EXCEPTIONS
