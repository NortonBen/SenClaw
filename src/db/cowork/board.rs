...
impl super::super::Db {
    ...
    pub fn get_cowork_board_entries(
        &self,
        workspace_id: &str,
        section: Option<&str>,
    ...
    pub fn update_cowork_board_entry(
        &self,
        id: &str,
        title: Option<&str>,
        content: Option<&str>,
        ...
        self.with_conn(|c| {
            if let Some(t) = title {
                c.execute(
                    "UPDATE cowork_board_entries SET title=?1,updated_at=?2 WHERE id=?3",
                    params![t, now, id],
                )?;
            }
            if let Some(ct) = content {
                c.execute(
                    "UPDATE cowork_board_entries SET content=?1,updated_at=?2 WHERE id=?3",
                    params![ct, now, id],
                )?;
            }
            if let Some(p) = pinned {
                c.execute(
                    "UPDATE cowork_board_entries SET pinned=?1,updated_at=?2 WHERE id=?3",
                    params![p as i64, now, id],
                )?;
            }
            if let Some(tg) = tags {
                c.execute(
                    "UPDATE cowork_board_entries SET tags=?1,updated_at=?2 WHERE id=?3",
                    params![tg, now, id],
                )?;
            }
            Ok(())
    ...
    pub fn delete_cowork_board_entry(&self, id: &str) -> Result<()> {
        self.with_conn(|c| {
            c.execute("DELETE FROM cowork_board_entries WHERE id=?1", params![id])?;
            Ok(())
        })
    }
}
