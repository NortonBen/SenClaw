...
pub fn schedule_mcp_config(
    ...
    cfg.env
        .insert("SENCLAW_DB_PATH".into(), db_path.to_owned());
    cfg.env
...
pub fn workspace_mcp_config(
    ...
    cfg.env
        .insert("SENCLAW_DEFAULT_WORKSPACE".into(), default_workspace.to_owned());
    let dirs_str = match allowed_work_dirs {
        None => String::new(),
        Some(list) => serde_json::to_string(list).unwrap_or_default(),
    };
    cfg.env
        .insert("SENCLAW_ALLOWED_WORK_DIRS".into(), dirs_str);
    cfg
}
...
pub fn memory_mcp_config(
    ...
    cfg.env
        .insert("SENCLAW_DB_PATH".into(), db_path.to_owned());
    cfg.env
        .insert("SENCLAW_FOLDER".into(), folder.to_owned());
    cfg.env
...
pub fn send_mcp_config(
    ...
    cfg.env
        .insert("SENCLAW_CHAT_JID".into(), chat_jid.to_owned());
    cfg.env
        .insert("SENCLAW_IS_ADMIN".into(), if is_admin { "1".into() } else { "0".into() });
    if let Some(tok) = bot_token {
        cfg.env
            .insert("SENCLAW_BOT_TOKEN".into(), tok.to_owned());
    }
    cfg.env
        .insert("SENCLAW_DB_PATH".into(), db_path.to_owned());
    cfg
...
pub fn virtual_mcp_config(
    ...
    cfg.env
        .insert("SENCLAW_AGENTS_CONFIG_DIR".into(), agents_config_dir.to_owned());
    cfg.env
        .insert("SENCLAW_ADMIN_FOLDER".into(), admin_folder.to_owned());
    cfg.env
        .insert("SENCLAW_DEFAULT_WORKSPACE".into(), default_workspace.to_owned());
...
pub fn feishu_wiki_mcp_config(app_id: &str, app_secret: &str, domain: Option<&str>) -> McpServerConfig {
    let mut cfg = McpServerConfig::new("senclaw-feishu-wiki", "feishu-wiki-server");
    cfg.env
        .insert("FEISHU_APP_ID".into(), app_id.to_owned());
    cfg.env
