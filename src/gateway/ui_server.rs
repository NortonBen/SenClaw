...
fn mcp_mgr(s: &UiState) -> Result<&McpManager, AppError> {
    s.mcp_manager
        .as_ref()
        .map(|m| m.as_ref())
...
async fn mcp_servers_list(State(s): State<Arc<UiState>>) -> Result<Json<serde_json::Value>, AppError> {
    let mgr = mcp_mgr(&s)?;
...
async fn mcp_servers_disconnect(
    State(s): State<Arc<UiState>>,
    AxumPath(name): AxumPath<String>,
) -> Result<Json<serde_json::Value>, AppError> {
    let mgr = mcp_mgr(&s)?;
