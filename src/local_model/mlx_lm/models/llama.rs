//! Llama-style dense decoder (RMSNorm + SwiGLU + RoPE, **no** attention Q/K norms).
//!
//! MLX checkpoints from `mlx-community` for **`llama`**, **`mistral`**, and **`qwen2`**
//! typically share the same `model.embed_tokens` / `model.layers.*` tensor naming as this
//! module expects. Use [`super::mistral`] / [`super::qwen2`] for thin re-exports.
...
pub struct ModelArgs {
    ...
    pub rope_theta: f32,
    /// Many HuggingFace / MLX configs omit this; it defaults to `hidden_size / num_attention_heads`.
    #[serde(default)]
    pub head_dim: i32,
    #[serde(default)]
...
impl<C> Module<ModelInput<'_, C>> for LlamaDecoder
where
    C: KeyValueCache,
{
    type Output = Array;
...
impl Model {
    pub fn new(args: ModelArgs) -> Result<Self, Exception> {
        let model = LlamaDecoder::new(&args)?;
    ...
    C: KeyValueCache,
{
    ...
    fn training_mode(&mut self, mode: bool) {
        <LlamaDecoder as Module<ModelInput<'_, C>>>::training_mode(&mut self.model, mode);
