...
use super::higgs_attn_utils::{AttentionMask, apply_rope, create_causal_mask};
use super::higgs_kv::{KeyValueCache, SteppingKeyValueCache};
...
impl Qwen3NextAttention {
    ...
    fn forward(
        ...
        let output = match view {
            super::higgs_kv::KvCacheView::TurboQuant(tq_view) if try_tq_decode => {
            ...
            other @ (super::higgs_kv::KvCacheView::Dense { .. }
            | super::higgs_kv::KvCacheView::TurboQuant(_)) => {
...
impl Qwen3NextCausalLM {
    ...
    pub fn make_cache_turbo(
        &self,
        kv_cache_config: super::higgs_turboquant_mlx::KvCacheConfig,
...
fn checkpoint_has_mtp_weights(model_path: &Path) -> Result<bool, ModelError> {
    ...
    for file_path in super::higgs_weights::collect_safetensors_files(model_path)? {
...
pub fn load_qwen3_next_model<P: AsRef<Path>>(
    ...
    super::higgs_weights::load_safetensors_weights(&mut model, model_path)?;
...
mod tests {
    use super::*;
    use super::higgs_kv::KeyValueCache;
...
fn forward_attention_sparse(
    ...
    x: &Array,
    positions: &Array,
    cache: &mut super::higgs_kv::SteppingKeyValueCache,
) -> Result<Array, Exception> {
    let shape = x.shape();
    let b = *shape
        .first()
    ...
    // Compute attention
    let output = super::higgs_attn_utils::scaled_dot_product_attention(
        queries,
        final_keys,
        final_values,
        attn.scale,
