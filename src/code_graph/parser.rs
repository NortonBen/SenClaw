...
impl CodeParser {
    pub fn parse_file(file_path: &str, source: &str, lang: Lang) -> Result<ParsedFile> {
        ...
        match lang {
            Lang::Rust => {
                walk_rust_nodes(root, src, &mut result, None);
                walk_rust_edges(root, src, &mut result, "");
            }
            Lang::TypeScript | Lang::JavaScript => {
                walk_ts_nodes(root, src, &mut result, None);
                walk_ts_edges(root, src, &mut result, "");
            }
            Lang::Python => {
                walk_python_nodes(root, src, &mut result, None);
                walk_python_edges(root, src, &mut result, "");
            }
            Lang::C | Lang::Cpp => {
                walk_c_nodes(root, src, &mut result, lang);
                walk_c_edges(root, src, &mut result, "");
            }
            Lang::CSharp => {
                walk_csharp_nodes(root, src, &mut result, None);
                walk_csharp_edges(root, src, &mut result, "");
            }
            Lang::Go => {
                walk_go_nodes(root, src, &mut result);
                walk_go_edges(root, src, &mut result, "");
            }
            Lang::Java => {
                walk_java_nodes(root, src, &mut result, None);
                walk_java_edges(root, src, &mut result, "");
            }
            Lang::Scala => {
                walk_scala_nodes(root, src, &mut result, None);
                walk_scala_edges(root, src, &mut result, "");
            }
            Lang::Ruby => {
                walk_ruby_nodes(root, src, &mut result, None);
                walk_ruby_edges(root, src, &mut result, "");
            }
            Lang::PHP => {
                walk_php_nodes(root, src, &mut result, None);
                walk_php_edges(root, src, &mut result, "");
            }
            Lang::Bash => {
                walk_bash_nodes(root, src, &mut result);
                walk_bash_edges(root, src, &mut result, "");
            }
            Lang::Haskell => {
                walk_haskell_nodes(root, src, &mut result);
                walk_haskell_edges(root, src, &mut result, "");
            }
            Lang::OCaml => {
                walk_ocaml_nodes(root, src, &mut result, None);
                walk_ocaml_edges(root, src, &mut result, "");
            }
            Lang::Julia => {
                walk_julia_nodes(root, src, &mut result, None);
                walk_julia_edges(root, src, &mut result, "");
            }
            Lang::Verilog => {
                walk_verilog_nodes(root, src, &mut result);
            }
            Lang::Agda => {
                walk_agda_nodes(root, src, &mut result);
            }
            _ => {}
        }
        ...
        "function_item" => {
            if let Some(name_n) = node.child_by_field_name("name") {
                ...
                push_node(
                    ...
                    name,
                    kind,
                    Some(rust_fn_sig(&node, src, impl_ctx)),
                    node,
                );
            }
                        ...
                        if matches!(m.kind(), "function_item" | "function_signature_item") {
                            if let Some(fn_n) = m.child_by_field_name("name") {
                                push_node(
                                    ...
                                    node_text(fn_n, src),
                                    NodeKind::Method,
                                    Some(rust_fn_sig(&m, src, Some(&tname))),
                                    m,
                                );
                            }
                        }
        ...
        "static_item" => {
            if let Some(n) = node.child_by_field_name("name") {
                ...
                let ty = node
                    ...
                    .map(|t| node_text(t, src))
                    .unwrap_or_default();
                let m = if has_child_kind(node, "mutable_specifier") {
                    "mut "
                } else {
                    ""
                };
...
fn walk_rust_edges(node: Node, src: &[u8], out: &mut ParsedFile, current_fn: &str) {
    ...
    for i in 0..node.child_count() {
        walk_rust_edges(node.child(i).unwrap(), src, out, &current);
    }
}
...
fn rust_fn_sig(node: &Node, src: &[u8], impl_ctx: Option<&str>) -> String {
    let asyncness = if has_child_kind(*node, "async") {
        "async "
    } else {
        ""
    };
...
fn walk_c_nodes(node: Node, src: &[u8], out: &mut ParsedFile, lang: Lang) {
    match node.kind() {
        ...
        "struct_specifier" | "union_specifier" => {
            if let Some(n) = node.child_by_field_name("name") {
                let name = node_text(n, src);
                let kind_str = if node.kind() == "union_specifier" {
                    "union"
                } else {
                    "struct"
                };
...
fn walk_c_edges(node: Node, src: &[u8], out: &mut ParsedFile, current_fn: &str) {
    ...
    for i in 0..node.child_count() {
        walk_c_edges(node.child(i).unwrap(), src, out, &current);
    }
}
...
fn walk_csharp_nodes(node: Node, src: &[u8], out: &mut ParsedFile, class_ctx: Option<&str>) {
        ...
        "method_declaration" => {
            if let Some(n) = node.child_by_field_name("name") {
                ...
                push_node(
                    ...
                    name.clone(),
                    kind,
                    Some(format!("{ret} {ctx}{name}{params}")),
                    node,
                );
            }
        }
    ...
    for i in 0..node.child_count() {
        walk_csharp_nodes(node.child(i).unwrap(), src, out, class_ctx);
    }
}
...
fn walk_csharp_edges(node: Node, src: &[u8], out: &mut ParsedFile, current_fn: &str) {
        ...
        "base_list" => {
            ...
            for i in 0..node.child_count() {
                ...
                if c.kind() == "identifier" || c.kind() == "generic_name" {
                    ...
                    let kind = if base.starts_with('I') {
                        EdgeKind::Implements
                    } else {
                        EdgeKind::Extends
                    };
    ...
    for i in 0..node.child_count() {
        walk_csharp_edges(node.child(i).unwrap(), src, out, &current);
    }
}
...
fn walk_go_nodes(node: Node, src: &[u8], out: &mut ParsedFile) {
    ...
    for i in 0..node.child_count() {
        walk_go_nodes(node.child(i).unwrap(), src, out);
    }
}
...
fn walk_go_edges(node: Node, src: &[u8], out: &mut ParsedFile, current_fn: &str) {
    ...
    for i in 0..node.child_count() {
        walk_go_edges(node.child(i).unwrap(), src, out, &current);
    }
}
...
fn walk_java_nodes(node: Node, src: &[u8], out: &mut ParsedFile, class_ctx: Option<&str>) {
    ...
    for i in 0..node.child_count() {
        walk_java_nodes(node.child(i).unwrap(), src, out, class_ctx);
    }
}
...
fn walk_java_edges(node: Node, src: &[u8], out: &mut ParsedFile, current_fn: &str) {
    ...
    for i in 0..node.child_count() {
        walk_java_edges(node.child(i).unwrap(), src, out, &current);
    }
}
...
fn walk_scala_nodes(node: Node, src: &[u8], out: &mut ParsedFile, class_ctx: Option<&str>) {
    ...
    for i in 0..node.child_count() {
        walk_scala_nodes(node.child(i).unwrap(), src, out, class_ctx);
    }
}
...
fn walk_scala_edges(node: Node, src: &[u8], out: &mut ParsedFile, current_fn: &str) {
    ...
    for i in 0..node.child_count() {
        walk_scala_edges(node.child(i).unwrap(), src, out, &current);
    }
}
...
fn walk_ts_nodes(node: Node, src: &[u8], out: &mut ParsedFile, class_ctx: Option<&str>) {
        ...
        "abstract_class_declaration" => {
            if let Some(n) = node.child_by_field_name("name") {
                ...
                push_node(
                    ...
                    cname.clone(),
                    NodeKind::Class,
                    Some(format!("abstract class {cname}")),
                    node,
                );
                for i in 0..node.child_count() {
                    walk_ts_nodes(node.child(i).unwrap(), src, out, Some(&cname.clone()));
                    ...
                    if let (Some(name_n), Some(val_n)) = (
                        ...
                        if matches!(val_n.kind(), "arrow_function" | "function_expression") {
                            ...
                            push_node(
                                ...
                                name.clone(),
                                kind,
                                Some(format!("const {name} = {prefix}({params}){ret} =>")),
                                child,
                            );
                        }
                    }
    ...
    for i in 0..node.child_count() {
        walk_ts_nodes(node.child(i).unwrap(), src, out, class_ctx);
    }
}
...
fn walk_ts_edges(node: Node, src: &[u8], out: &mut ParsedFile, current_fn: &str) {
    ...
    for i in 0..node.child_count() {
        walk_ts_edges(node.child(i).unwrap(), src, out, &current);
    }
}
        ...
        "function_definition" | "async_function_definition" => {
            if let Some(n) = node.child_by_field_name("name") {
                ...
                push_node(
                    ...
                    name.clone(),
                    kind,
                    Some(format!("{prefix}def {name}{params}{ret}")),
                    node,
                );
            }
            for i in 0..node.child_count() {
                walk_python_nodes(node.child(i).unwrap(), src, out, class_ctx);
            }
            return;
        }
        "class_definition" => {
            if let Some(n) = node.child_by_field_name("name") {
                ...
                let sig = if bases.is_empty() {
                    ...
                    format!("class {cname}({bases})")
                };
                push_node(out, cname.clone(), NodeKind::Class, Some(sig), node);
                for i in 0..node.child_count() {
                    walk_python_nodes(node.child(i).unwrap(), src, out, Some(&cname.clone()));
                }
                return;
            }
        }
...
fn walk_python_edges(node: Node, src: &[u8], out: &mut ParsedFile, current_fn: &str) {
    ...
    match node.kind() {
        ...
        "import_from_statement" => {
            let module = node
                ...
                .unwrap_or_default();
            if !module.is_empty() {
                let to_file = if module.starts_with('.') {
                    None
                } else {
                    resolve_python_import(&out.file_path, &module)
                };
                out.edges.push(RawEdge {
                    from_name: out.file_path.clone(),
                    to_name: module,
        ...
        "class_definition" => {
            if let (Some(name_n), Some(args_n)) = (
                node.child_by_field_name("name"),
                ...
                for i in 0..args_n.child_count() {
                    ...
                    match arg.kind() {
                        ...
                        "attribute" => {
                            if let Some(a) = arg.child_by_field_name("attribute") {
                                push_edge(
                                    out,
                                    &cname,
                                    node_text(a, src),
    ...
    for i in 0..node.child_count() {
        walk_python_edges(node.child(i).unwrap(), src, out, &current);
    }
}
...
fn walk_ruby_nodes(node: Node, src: &[u8], out: &mut ParsedFile, class_ctx: Option<&str>) {
    match node.kind() {
        ...
        "class" | "singleton_class" => {
            if let Some(n) = node.child_by_field_name("name") {
                ...
                for i in 0..node.child_count() {
                    walk_ruby_nodes(node.child(i).unwrap(), src, out, Some(&name.clone()));
                }
                return;
            }
        }
    ...
    for i in 0..node.child_count() {
        walk_ruby_nodes(node.child(i).unwrap(), src, out, class_ctx);
    }
}
...
fn walk_ruby_edges(node: Node, src: &[u8], out: &mut ParsedFile, current_fn: &str) {
    ...
    for i in 0..node.child_count() {
        walk_ruby_edges(node.child(i).unwrap(), src, out, &current);
    }
}
...
fn walk_php_nodes(node: Node, src: &[u8], out: &mut ParsedFile, class_ctx: Option<&str>) {
    match node.kind() {
        ...
        "class_declaration" | "enum_declaration" => {
            if let Some(n) = node.child_by_field_name("name") {
                ...
                push_node(out, name.clone(), kind, Some(format!("class {name}")), node);
                for i in 0..node.child_count() {
                    walk_php_nodes(node.child(i).unwrap(), src, out, Some(&name.clone()));
                }
                return;
            }
        }
    ...
    for i in 0..node.child_count() {
        walk_php_nodes(node.child(i).unwrap(), src, out, class_ctx);
    }
}
...
fn walk_php_edges(node: Node, src: &[u8], out: &mut ParsedFile, current_fn: &str) {
    ...
    for i in 0..node.child_count() {
        walk_php_edges(node.child(i).unwrap(), src, out, &current);
    }
}
...
fn walk_bash_nodes(node: Node, src: &[u8], out: &mut ParsedFile) {
    ...
    for i in 0..node.child_count() {
        walk_bash_nodes(node.child(i).unwrap(), src, out);
    }
}
...
fn walk_bash_edges(node: Node, src: &[u8], out: &mut ParsedFile, current_fn: &str) {
    ...
    for i in 0..node.child_count() {
        walk_bash_edges(node.child(i).unwrap(), src, out, &current);
    }
}
...
fn walk_haskell_edges(node: Node, src: &[u8], out: &mut ParsedFile, current_fn: &str) {
    ...
    for i in 0..node.child_count() {
        walk_haskell_edges(node.child(i).unwrap(), src, out, &current);
    }
}
...
fn walk_ocaml_nodes(node: Node, src: &[u8], out: &mut ParsedFile, module_ctx: Option<&str>) {
    match node.kind() {
        ...
        "module_binding" => {
            if let Some(n) = node.child_by_field_name("name") {
                ...
                for i in 0..node.child_count() {
                    walk_ocaml_nodes(node.child(i).unwrap(), src, out, Some(&name.clone()));
                }
                return;
            }
        }
    ...
    for i in 0..node.child_count() {
        walk_ocaml_nodes(node.child(i).unwrap(), src, out, module_ctx);
    }
}
...
fn walk_ocaml_edges(node: Node, src: &[u8], out: &mut ParsedFile, current_fn: &str) {
    ...
    for i in 0..node.child_count() {
        walk_ocaml_edges(node.child(i).unwrap(), src, out, &current);
    }
}
...
fn walk_julia_nodes(node: Node, src: &[u8], out: &mut ParsedFile, struct_ctx: Option<&str>) {
    match node.kind() {
        "function_definition" | "short_function_definition" => {
            if let Some(n) = node.child_by_field_name("name") {
                let name = node_text(n, src);
                let kind = if struct_ctx.is_some() {
                    NodeKind::Method
                } else {
                    NodeKind::Function
                };
        ...
        "struct_definition" | "mutable_struct_definition" => {
            if let Some(n) = node.child_by_field_name("name") {
                ...
                for i in 0..node.child_count() {
                    walk_julia_nodes(node.child(i).unwrap(), src, out, Some(&name.clone()));
                }
                return;
            }
        }
        "abstract_definition" => {
            if let Some(n) = node.child_by_field_name("name") {
                ...
                push_node(
                    ...
                    name.clone(),
                    NodeKind::Type,
                    Some(format!("abstract type {name}")),
                    node,
                );
            }
        }
    ...
    for i in 0..node.child_count() {
        walk_julia_nodes(node.child(i).unwrap(), src, out, struct_ctx);
    }
}
...
fn walk_julia_edges(node: Node, src: &[u8], out: &mut ParsedFile, current_fn: &str) {
    ...
    match node.kind() {
        ...
        "using_statement" | "import_statement" => {
            let text = node_text(node, src);
            let kw = if node.kind() == "using_statement" {
                "using"
            } else {
                "import"
            };
    ...
    for i in 0..node.child_count() {
        walk_julia_edges(node.child(i).unwrap(), src, out, &current);
    }
}
...
fn walk_verilog_nodes(node: Node, src: &[u8], out: &mut ParsedFile) {
    ...
    for i in 0..node.child_count() {
        walk_verilog_nodes(node.child(i).unwrap(), src, out);
    }
}
...
fn walk_agda_nodes(node: Node, src: &[u8], out: &mut ParsedFile) {
    ...
    for i in 0..node.child_count() {
        walk_agda_nodes(node.child(i).unwrap(), src, out);
    }
}
