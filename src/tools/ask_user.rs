...
use crate::zen_core::{AskQuestionItem, AskQuestionOption, AskQuestionRequestData, EngineEvent, Tool, ToolContext, ToolOutput, ToolResultMessage};
...
impl Tool for AskUserTool {
    ...
    async fn validate_input(
        ...
        for q in questions {
            ...
            if header.len() > 12 {
                return Err(format!("Header \"{header}\" exceeds maximum length of 12 characters"));
            }
        }
        Ok(())
    }
    ...
    async fn call(
        ...
        let questions_arr = input.get("questions")
            .and_then(|v| v.as_array())
            .cloned()
            .unwrap_or_default();

        let questions: Vec<AskQuestionItem> = questions_arr.iter().map(|q| {
            AskQuestionItem {
                question: q.get("question").and_then(|s| s.as_str()).unwrap_or("").to_string(),
                header: q.get("header").and_then(|s| s.as_str()).unwrap_or("").to_string(),
                options: q.get("options").and_then(|a| a.as_array()).map(|arr| {
                    arr.iter().map(|o| AskQuestionOption {
                        label: o.get("label").and_then(|s| s.as_str()).unwrap_or("").to_string(),
                        description: o.get("description").and_then(|s| s.as_str()).unwrap_or("").to_string(),
                    }).collect()
                }).unwrap_or_default(),
                multi_select: q.get("multiSelect").and_then(|b| b.as_bool()).unwrap_or(false),
            }
        }).collect();

        let event_bus = ctx.event_bus.ok_or_else(|| anyhow::anyhow!("EventBus not available"))?;
        let response_registry = ctx.response_registry.ok_or_else(|| anyhow::anyhow!("ResponseRegistry not available"))?;
    ...
    fn gen_tool_result_message(&self, data: &Value, _input: &Value) -> ToolResultMessage {
        ...
        if let Some(answers) = answers.and_then(|a| a.as_object()) {
            if !answers.is_empty() {
                ...
                let content = answers.iter()
                    .map(|(q, a)| format!("  {}: -> {}", q, a.as_str().unwrap_or("")))
                    .collect::<Vec<_>>()
                    .join("\n");
                return ToolResultMessage {
                    title: "User Response".into(),
                    summary: if count == 1 { "Got 1 answer".into() } else { format!("Got {count} answers") },
        ...
        ToolResultMessage {
            ...
            summary: format!("Asked {questions} question{}", if questions == 1 { "" } else { "s" }),
            content: data.clone(),
        }
    }
    ...
    fn get_display_title(&self, input: &Value) -> String {
        let questions = input.get("questions").and_then(|v| v.as_array());
        match questions.and_then(|q| q.first()).and_then(|q| q.get("header")).and_then(|h| h.as_str()) {
...
fn format_answers(questions: &[AskQuestionItem], answers: &HashMap<String, String>) -> String {
    let parts: Vec<String> = answers.iter()
        .map(|(q, a)| format!("\"{}\"=\"{}\"", q, a))
        .collect();
    if parts.is_empty() {
        let qlist: Vec<String> = questions.iter().map(|q| format!("- {}", q.question)).collect();
        format!("Waiting for user to answer:\n{}", qlist.join("\n"))
    } else {
        format!("User's answers: {}. Continue with this information.", parts.join(", "))
    }
}
