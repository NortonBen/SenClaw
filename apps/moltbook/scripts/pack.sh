#!/usr/bin/env bash
# Build SenClaw Moltbook and assemble an installable Space-App zip.
#
#   release/                 <- staged, flat install layout
#     moltbook               (release binary; manifest runtime.start = ./moltbook)
#     senclaw-manifest.json
#     skills/                (moltbook-browse, moltbook-participate)
#     personas/              (molty, molty-observer)
#     web_dist/              (built React UI — main.rs serves web_dist next to the binary)
#   moltbook-app.zip         <- the artifact you install in SenClaw
#
# Usage: apps/moltbook/scripts/pack.sh [--skip-build]
set -euo pipefail

APP_DIR="$(cd "$(dirname "$0")/.." && pwd)"
ROOT="$(cd "$APP_DIR/../.." && pwd)"
REL="$APP_DIR/release"
ZIP="$APP_DIR/moltbook-app.zip"
BIN="$ROOT/target/release/moltbook"

if [[ "${1:-}" != "--skip-build" ]]; then
  echo "==> building web UI"
  ( cd "$APP_DIR/web" && npm install --silent && npm run build )
  echo "==> building release binary"
  ( cd "$ROOT" && cargo build -p moltbook --release )
fi
