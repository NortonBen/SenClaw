#!/usr/bin/env bash
# Build SenClaw Cafe (Quán Cafe) and assemble an installable Space-App zip.
#
#   release/                 <- staged, flat install layout
#     cafe                   (release binary; manifest runtime.start = ./cafe)
#     senclaw-manifest.json
#     skills/ personas/
#     web_dist/              (built React UI — main.rs serves web_dist next to the binary)
#   cafe-app.zip             <- the artifact you install in SenClaw
#
# Usage: apps/cafe/scripts/pack.sh [--skip-build]
set -euo pipefail

APP_DIR="$(cd "$(dirname "$0")/.." && pwd)"
ROOT="$(cd "$APP_DIR/../.." && pwd)"
REL="$APP_DIR/release"
ZIP="$APP_DIR/cafe-app.zip"
BIN="$ROOT/target/release/cafe"

if [[ "${1:-}" != "--skip-build" ]]; then
  echo "==> building web UI"
  ( cd "$APP_DIR/web" && npm install --silent && npm run build )
  echo "==> building release binary"
  ( cd "$ROOT" && cargo build -p cafe --release )
fi
