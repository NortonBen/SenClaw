...
pub fn routes() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route("/customers/:id/organizations", get(customer_orgs).post(link_org))
        ...
        .route("/deals/:id/services", get(deal_services).post(attach_service))
...
async fn list_orgs(
    ...
    let orgs = s
        .db
        .list_organizations(q.q.as_deref(), q.kind.as_deref(), clamp(q.limit, 200))
        .map_err(server)?;
...
async fn get_org(
    ...
    Ok(Json(json!({ "organization": org, "contacts": contacts, "deals": deals })))
}
...
async fn update_org(
    ...
    s.db.update_organization(id, &patch, now_ts()).map_err(bad)?;
    let org = s.db.get_organization(id).map_err(server)?;
    Ok(Json(json!({ "organization": org })))
}
...
async fn org_contacts(
    ...
    Ok(Json(json!({ "contacts": s.db.contacts_of_org(id).map_err(server)? })))
}
...
async fn org_deals(
    ...
    Ok(Json(json!({ "deals": s.db.deals_of_organization(id).map_err(server)? })))
}
...
async fn customer_orgs(
    ...
    Ok(Json(json!({ "organizations": s.db.orgs_of_customer(id).map_err(server)? })))
}
        ...
        (None, Some(name)) if !name.trim().is_empty() => {
            match s.db.find_organization_by_name(name).map_err(server)? {
                ...
                None => s
                    .db
                    .create_organization(
                        &OrganizationInput { name: name.trim().to_string(), ..Default::default() },
                        now,
                    )
                    .map_err(bad)?,
            }
...
async fn unlink_org(
    ...
    s.db.unlink_customer_org(id, org_id, now_ts()).map_err(not_found)?;
    Ok(Json(json!({ "organizations": s.db.orgs_of_customer(id).map_err(server)? })))
}
...
async fn list_services(
    ...
    let services = s
        .db
        .list_services(q.q.as_deref(), q.kind.as_deref(), q.active_only, clamp(q.limit, 200))
        .map_err(server)?;
...
async fn create_service(
    ...
    Ok(Json(json!({ "service": s.db.get_service(id).map_err(server)? })))
}
...
async fn deal_services(
    ...
    Ok(Json(json!({ "services": items, "quantity": quantity, "total": total })))
}
...
async fn detach_service(
    ...
    s.db.detach_service(id, service_id, now_ts()).map_err(not_found)?;
    Ok(Json(json!({ "services": s.db.services_of_deal(id).map_err(server)? })))
}
