...
use crate::db::Db;
use crate::db_inbox::{Channel, ChannelPatch};
...
pub async fn poll(db: &Arc<Db>, ch: &Channel) -> Result<Vec<Inbound>, String> {
    ...
    let since: i64 = ch.cursor.parse().unwrap_or_else(|_| now_ms() - 7 * 24 * 3600 * 1000);
    let recent =
        zalo_get(db, ch, OA_V2, "listrecentchat", &json!({ "offset": 0, "count": 10 })).await?;
...
/// Send a text message via the v3.0 customer-service message endpoint.
...
/// Returns the new access token. Persisting is not optional: Zalo invalidates
/// the old refresh token the moment it issues a new one, so a pair we fail to
/// store is a channel that can never authenticate again.
async fn refresh_token(db: &Arc<Db>, ch: &Channel) -> Result<String, String> {
    ...
    let _ = db.update_channel_cfg(
        ch.id,
        &ChannelPatch { config: Some(new_cfg), ..Default::default() },
    );
    Ok(new_access)
}
...
mod tests {
    ...
    #[test]
    fn cursor_advances_past_oa_messages_too() {
        // The OA's own reply must still move the cursor, or every poll would
        // re-scan it forever.
        let list = vec![json!({ "src": 0, "message": "OA trả lời", "time": 900 })];
        let (msgs, newest) = normalize_messages("u1", "An", &list, 200);
