#!/usr/bin/env bash
# Build SenClaw TikTok Downloader and assemble an installable Space-App zip.
#
#   release/                 <- staged, flat install layout
#     tiktok-dl              (release binary; manifest runtime.start = ./tiktok-dl)
#     senclaw-manifest.json
#     skills/ personas/
#     web_dist/              (built React UI — main.rs serves web_dist next to the binary)
#   tiktok-dl-app.zip        <- the artifact you install in SenClaw
#
# Usage: apps/tiktok-dl/scripts/pack.sh [--skip-build]
set -euo pipefail

APP_DIR="$(cd "$(dirname "$0")/.." && pwd)"
ROOT="$(cd "$APP_DIR/../.." && pwd)"
REL="$APP_DIR/release"
ZIP="$APP_DIR/tiktok-dl-app.zip"
BIN="$ROOT/target/release/tiktok-dl"

if [[ "${1:-}" != "--skip-build" ]]; then
  echo "==> building web UI"
  ( cd "$APP_DIR/web" && npm install --silent && npm run build )
  echo "==> building release binary"
  ( cd "$ROOT" && cargo build -p tiktok-dl --release )
fi
...
rm -rf "$REL" "$ZIP"
mkdir -p "$REL"
cp "$BIN" "$REL/tiktok-dl"
chmod +x "$REL/tiktok-dl"
