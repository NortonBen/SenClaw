#!/usr/bin/env bash
# Build SenClaw Video Cloner and assemble an installable Space-App zip.
#
#   release/                  <- staged, flat install layout
#     video-cloner            (release binary; manifest runtime.start = ./video-cloner)
...
#   video-cloner-app.zip      <- the artifact you install in SenClaw
#
# Usage: apps/video-cloner/scripts/pack.sh [--skip-build]
set -euo pipefail

APP_DIR="$(cd "$(dirname "$0")/.." && pwd)"
ROOT="$(cd "$APP_DIR/../.." && pwd)"
REL="$APP_DIR/release"
ZIP="$APP_DIR/video-cloner-app.zip"
BIN="$ROOT/target/release/video-cloner"

if [[ "${1:-}" != "--skip-build" ]]; then
  echo "==> building web UI"
  ( cd "$APP_DIR/web" && npm install --silent && npm run build )
  echo "==> building release binary"
  ( cd "$ROOT" && cargo build -p video-cloner --release )
fi
...
rm -rf "$REL" "$ZIP"
mkdir -p "$REL"
cp "$BIN" "$REL/video-cloner"
chmod +x "$REL/video-cloner"
