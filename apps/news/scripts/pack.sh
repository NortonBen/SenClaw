#!/usr/bin/env bash
# Build SenClaw News (Tin Tức) and assemble an installable Space-App zip.
#
#   release/                 <- staged, flat install layout
#     news                   (release binary; manifest runtime.start = ./news)
#     senclaw-manifest.json
#     skills/ personas/
#     web_dist/              (built React UI — main.rs serves web_dist next to the binary)
#   news-app.zip             <- the artifact you install in SenClaw
#
# Usage: apps/news/scripts/pack.sh [--skip-build]
set -euo pipefail

APP_DIR="$(cd "$(dirname "$0")/.." && pwd)"
ROOT="$(cd "$APP_DIR/../.." && pwd)"
REL="$APP_DIR/release"
ZIP="$APP_DIR/news-app.zip"
BIN="$ROOT/target/release/news"

if [[ "${1:-}" != "--skip-build" ]]; then
  echo "==> building web UI"
  ( cd "$APP_DIR/web" && npm install --silent && npm run build )
  echo "==> building release binary"
  ( cd "$ROOT" && cargo build -p news --release )
fi
