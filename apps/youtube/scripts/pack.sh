#!/usr/bin/env bash
# Build SenClaw YouTube and assemble an installable Space-App zip.
#
#   release/                 <- staged, flat install layout
#     youtube               (release binary; manifest runtime.start = ./youtube)
#     senclaw-manifest.json
#     skills/               (youtube-browse)
#     personas/             (youtube-operator)
#     extension/            (unpacked Chrome extension — user loads it manually)
#     web_dist/             (built React UI — main.rs serves web_dist next to the binary)
#   youtube-app.zip          <- the artifact you install in SenClaw
#
# Usage: apps/youtube/scripts/pack.sh [--skip-build]
set -euo pipefail

APP_DIR="$(cd "$(dirname "$0")/.." && pwd)"
ROOT="$(cd "$APP_DIR/../.." && pwd)"
REL="$APP_DIR/release"
ZIP="$APP_DIR/youtube-app.zip"
BIN="$ROOT/target/release/youtube"

if [[ "${1:-}" != "--skip-build" ]]; then
  echo "==> building web UI"
  ( cd "$APP_DIR/web" && npm install --silent && npm run build )
  echo "==> building release binary"
  ( cd "$ROOT" && cargo build -p youtube --release )
fi
