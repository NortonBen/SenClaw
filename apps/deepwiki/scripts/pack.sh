#!/usr/bin/env bash
# Build DeepWiki and assemble an installable Space-App zip.
#
#   release/                 <- staged, flat install layout
#     deepwiki               (release binary; manifest runtime.start = ./deepwiki)
#     senclaw-manifest.json
#     skills/                (deepwiki-generate / -ask / -explore / -review)
#     personas/              (wiki-author, codebase-guide, code-explorer, impact-analyst)
#     web_dist/              (built React UI — main.rs serves web_dist next to the binary)
#   deepwiki-app.zip         <- the artifact you install in SenClaw
#
# Usage: apps/deepwiki/scripts/pack.sh [--skip-build]
set -euo pipefail

APP_DIR="$(cd "$(dirname "$0")/.." && pwd)"
ROOT="$(cd "$APP_DIR/../.." && pwd)"
REL="$APP_DIR/release"
ZIP="$APP_DIR/deepwiki-app.zip"
BIN="$ROOT/target/release/deepwiki"

if [[ "${1:-}" != "--skip-build" ]]; then
  echo "==> building web UI"
  ( cd "$APP_DIR/web" && npm install --silent && npm run build )
  echo "==> building release binary"
  ( cd "$ROOT" && cargo build -p deepwiki --release )
fi
