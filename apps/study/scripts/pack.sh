#!/usr/bin/env bash
# Build SenClaw Study and assemble an installable Space-App zip.
#
#   release/              <- staged, flat install layout
#     study               (release binary; manifest runtime.start = ./study)
#     senclaw-manifest.json
#     skills/             (study-coach)
#     personas/           (study-coach)
#     web_dist/           (built React UI — main.rs serves web_dist next to the binary)
#   study-app.zip         <- the artifact you install in SenClaw
#
# Usage: apps/study/scripts/pack.sh [--skip-build]
set -euo pipefail

APP_DIR="$(cd "$(dirname "$0")/.." && pwd)"
ROOT="$(cd "$APP_DIR/../.." && pwd)"
REL="$APP_DIR/release"
ZIP="$APP_DIR/study-app.zip"
BIN="$ROOT/target/release/study"

if [[ "${1:-}" != "--skip-build" ]]; then
  echo "==> building web UI"
  ( cd "$APP_DIR/web" && npm install --silent && npm run build )
  echo "==> building release binary"
  ( cd "$ROOT" && cargo build -p study --release )
fi
