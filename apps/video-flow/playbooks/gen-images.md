---
name: gen-images
description: Tạo ảnh scene cho toàn bộ scene trong video
triggers:
  - sinh ảnh scene
  - tạo ảnh cảnh
  - gen ảnh
...
```bash
PROJ_OUT=$(curl -s http://127.0.0.1:4460/api/projects/<PID>/output-dir)
...
```bash
curl -s http://127.0.0.1:4460/api/projects/<PID>/characters
```
...
```bash
curl -X POST http://127.0.0.1:4460/api/requests/batch \
...
```bash
curl -s "http://127.0.0.1:4460/api/requests/batch-status?video_id=<VID>&type=GENERATE_IMAGE"
# Wait for: "done": true
```
...
```bash
curl -s "http://127.0.0.1:4460/api/requests/batch-status?video_id=<VID>&type=EDIT_IMAGE"
# Wait for: "done": true
```
