---
name: creative-mix
description: Gợi ý kỹ thuật mix video sáng tạo (chain, multi-angle)
triggers:
  - ý tưởng sáng tạo
  - mix video
  - biến tấu
...
**Camera angle ideas for edits** (see `/camera-guide`):
...
```bash
curl -X DELETE "http://127.0.0.1:4460/api/scenes?video_id=<VID>&source=system"
```
