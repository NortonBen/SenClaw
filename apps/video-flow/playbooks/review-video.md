---
name: review-video
description: Review chất lượng video scene bằng Claude Vision
triggers:
  - kiểm tra chất lượng
  - review video
  - xem lại clip
...
Usage: `/review-video <video_id> [--mode light|deep]`
...
# Verify video exists
curl -s http://127.0.0.1:4460/api/videos/<VID>
```
...
**ABORT** if any scene is missing a completed video — tell user to run `/gen-videos` first.

## Step 3: Run review via API

```bash
curl -X POST "http://127.0.0.1:4460/api/videos/<VID>/review?project_id=<PID>&mode=light&orientation=${ORI}"
```
...
**Poll until complete:**
```bash
curl -s http://127.0.0.1:4460/api/requests/<RID>
...
```
Then run `/gen-videos <PID> <VID>` after image is complete.

### Acceptable with good segments
Note `usable_segments` time ranges for manual editing. Use `/concat` and trim in post.
...
```bash
curl -X POST "http://127.0.0.1:4460/api/videos/<VID>/review?project_id=<PID>&mode=deep"
```
...
- Excellent/Good → "Ready for `/concat <VID>`"
- Acceptable → "Note usable segments, trim in post"
- Poor/Unusable → "Run `/gen-images <PID> <VID>` to regenerate, then `/gen-videos <PID> <VID>`"
