---
name: create-project
description: Tạo project Google Flow mới với story, material, nhân vật và scene
triggers:
  - tạo project
  - dự án mới
  - bắt đầu video mới
...
```bash
curl -X POST http://127.0.0.1:4460/api/videos \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST http://127.0.0.1:4460/api/scenes \
  -H "Content-Type: application/json" \
...
See `camera-guide.md` for full camera language reference.
...
See `camera-guide.md` for full Veo 3 camera/lighting/audio vocabulary and prompt template.
...
See `gen-narrator.md` for word count limits per language and narrative arc guide.
...
```bash
curl -X PATCH http://127.0.0.1:4460/api/scenes/<SID> \
  -H "Content-Type: application/json" \
  -d '{
...
**Workflow:** create scenes → review all prompts → PATCH to improve → then run /gen-refs and /gen-images. Scenes are mutable — update freely before generation starts.
