---
name: monitor
description: Theo dõi pipeline theo thời gian, Telegram và auto-download upscale
triggers:
  - theo dõi tiến độ
  - đang chạy tới đâu
  - monitor
...
Usage: `/monitor [project_id] [orientation] [--download] [--interval N]`
...
```bash
curl -s http://127.0.0.1:4460/health
...
```bash
curl -s http://127.0.0.1:4460/api/projects/<PID>/output-dir
...
```bash
curl -s "http://127.0.0.1:4460/api/videos?project_id=<PID>"
# Use first video → id, get scene count

curl -s "http://127.0.0.1:4460/api/scenes?video_id=<VID>"
# Total scene count = N
```
...
curl -s "http://127.0.0.1:4460/api/requests?status=PENDING" > /tmp/gla_pending.json &
curl -s "http://127.0.0.1:4460/api/requests?status=PROCESSING" > /tmp/gla_processing.json &
curl -s "http://127.0.0.1:4460/api/requests?status=FAILED" > /tmp/gla_failed.json &
wait
```
...
| Scenario | Next Skill |
|----------|------------|
| All videos done, no 4K | `/concat` |
| All 4K downloaded | `/concat --4k` |
| Need narration first | `/gen-narrator` → `/concat --with-tts` |
| Upload to YouTube | `/youtube-seo` → `/youtube-upload` |
