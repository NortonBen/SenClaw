---
name: camera-guide
description: Hướng dẫn prompt camera và Veo 3 cho video điện ảnh
triggers:
  - góc máy
  - chuyển động camera
  - camera
...
### Scene-Level Camera Movement Field

- Store camera movement in `scene.camera_movement` as a dedicated field.
- Keep `scene.video_prompt` focused on action beats, lighting, and audio.
- Worker auto-appends `camera_movement` as a separate sentence during video generation.
- Recommended format for `camera_movement`: full standalone sentence (example: `The camera slowly dollies in from medium shot to close-up.`).
- Update via `PATCH /api/scenes/{sid}` when refining timing or framing.
