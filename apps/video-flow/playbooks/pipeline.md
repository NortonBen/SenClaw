---
name: pipeline
description: Tự động phát hiện trạng thái project và chạy full pipeline (batch, upscale, TTS)
triggers:
  - chạy pipeline
  - làm video từ đầu
  - full pipeline
...
# pipeline — Smart Full-Pipeline Orchestrator
...
Usage: `/pipeline [project_id] [orientation] [options]`
...
Examples:
- `/pipeline` — detect state and continue most recent project
- `/pipeline --upscale --tts --download --notify` — full run with all options
- `/pipeline <project_id> HORIZONTAL --upscale --download` — explicit project + orientation
...
```bash
# Most recent project
curl -s http://127.0.0.1:4460/api/projects
# Use last item → {id, name}

# Get video + scenes
VID=$(curl -s "http://127.0.0.1:4460/api/videos?project_id=<PID>" | python3 -c "import sys,json; print(json.load(sys.stdin)[0]['id'])")
curl -s "http://127.0.0.1:4460/api/scenes?video_id=$VID" > /tmp/gla_scenes.json
curl -s "http://127.0.0.1:4460/api/projects/<PID>/characters" > /tmp/gla_chars.json
curl -s "http://127.0.0.1:4460/api/projects/<PID>/output-dir" > /tmp/gla_outdir.json
```
...
for CID in <missing_ids>:
  curl -X POST http://127.0.0.1:4460/api/requests \
...
Only run after all videos COMPLETED. Uses `/review-video` to catch AI generation errors before upscaling.

```bash
# Run light review on all completed videos
curl -X POST "http://127.0.0.1:4460/api/videos/<VID>/review?project_id=<PID>&mode=light&orientation=<ORIENTATION>"
# Poll until complete
```
...
```bash
# Check templates
# Giọng đọc lấy từ SenClaw Settings → TTS (không có template riêng trong app này)
curl -s -X POST http://127.0.0.1:4460/api/mcp/message -H 'content-type: application/json' -d '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"vf_tts_status","arguments":{}}}'
# Pick template name (e.g. vi_male_narrator)

# Trigger narration for video
curl -X POST http://127.0.0.1:4460/api/videos/<VID>/narrate \
  -H "Content-Type: application/json" \
...
**If no template exists:** Pause and instruct user to run `/gen-tts-template` or `/import-voice` first.
...
Run after UPSCALE + DOWNLOAD + TTS all complete. Delegates to `/concat`.

Invoke: `/concat --4k --with-tts` (or appropriate flags based on what was run).
...
| Chưa cài model TTS | `vf_tts_status` không có model nào installed | Dừng; bảo người dùng cài trong SenClaw Settings → TTS |
...
```
[pipeline] Operation Absolute Resolve - Cinematic Edition [HORIZONTAL]
...
| Worker treo | Khởi động lại app trong SenClaw (Restart); lúc boot app tự đối soát request kẹt PROCESSING |
...
|---|---|
| Mix TTS into videos | `/concat --with-tts` |
| Add branding watermark | `/brand-logo` |
| Generate YouTube SEO | `/youtube-seo` |
