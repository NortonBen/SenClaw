---
name: gen-refs
description: Tạo ảnh tham chiếu cho mọi entity trong project
triggers:
  - ảnh tham chiếu
  - reference image
  - ảnh nhân vật
...
If no project_id provided, list projects via `GET /api/projects` and ask which one.
...
```bash
curl -s "http://127.0.0.1:4460/api/requests/batch-status?project_id=<PID>&type=GENERATE_CHARACTER_IMAGE"
# Wait for: "done": true
...
curl -s -X PATCH http://127.0.0.1:4460/api/characters/<CID> \
  -H "Content-Type: application/json" \
  -d '{"image_prompt": "<rewritten prompt with left side three-quarter profile view>"}'

# Regenerate with new prompt
curl -s -X POST http://127.0.0.1:4460/api/requests \
  -H "Content-Type: application/json" \
