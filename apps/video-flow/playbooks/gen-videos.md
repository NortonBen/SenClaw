---
name: gen-videos
description: Sinh video cho mọi scene đã có ảnh
triggers:
  - sinh video
  - tạo clip
  - gen video
...
**ABORT** if any scene is missing `${ori}_image_media_id` (UUID) or `${ori}_image_status` != `"COMPLETED"`. Tell user to run `/gen-images` first.
...
```bash
curl -s "http://127.0.0.1:4460/api/requests/batch-status?video_id=<VID>&type=GENERATE_VIDEO"
# Wait for: "done": true
