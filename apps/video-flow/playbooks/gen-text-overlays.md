---
name: gen-text-overlays
description: Sinh text_overlays.json từ narrator cho concat-fit-narrator
triggers:
  - chữ trên video
  - text overlay
  - phụ đề
...
# gen-text-overlays — Generate Text Overlays from Narrator Text

Analyze narrator text for each scene and extract key data points (dates, locations, statistics, milestones, costs) to create `text_overlays.json` for `/concat-fit-narrator`.

Usage: `/gen-text-overlays <video_id> [--language vi]`
...
# Get project_id
curl -s http://127.0.0.1:4460/api/projects/<PID>
