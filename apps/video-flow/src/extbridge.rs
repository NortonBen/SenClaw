//! Extension bridge — port of `internal/bridge/extbridge.go`. A dedicated WS
//! server on :9222 (the unchanged Chrome extension dials ws://127.0.0.1:9222).
//! Request/response RPC over the socket: outbound commands carry an `id`, the
//! extension answers with a message bearing the same `id` (over WS or via
//! `POST /api/ext/callback`), which resolves a pending oneshot.
...
impl ExtBridge {
    ...
    /// Register interest in a callback id — returns the receiver to await.
    ...
    /// Deliver a callback message to whoever is waiting on `id`.
    ...
    pub fn send(&self, id: &str, method: &str, params: Value) -> Result<(), String> {
        ...
        match guard.as_ref() {
            ...
            _ => Err("Extension not connected".to_string()),
        }
    }

    /// send + await the correlated callback with a timeout. The Go
    /// `RegisterPending`/`Send`/`WaitPending` triple in one call.
    pub async fn call(&self, method: &str, params: Value, timeout: Duration) -> Result<Value, String> {
        ...
        match tokio::time::timeout(timeout, rx).await {
            ...
            Err(_) => {
                self.cancel_pending(&id);
                Err(format!("extension call {method} timed out after {}s", timeout.as_secs()))
            }
        }
    }
    ...
    pub async fn serve(&self, socket: WebSocket) {
        ...
        *self.inner.connected_since.lock().unwrap() = Some(Instant::now());
        ...
        writer.abort();
        // Only mark disconnected if no newer connection replaced ours.
        {
            let mut guard = self.inner.conn_tx.lock().unwrap();
...
/// Run the dedicated extension WS server (Go: ServeWS on :9222, route "/").
pub async fn serve_ws(bridge: ExtBridge, port: u16) {
    ...
    match tokio::net::TcpListener::bind(format!("0.0.0.0:{port}")).await {
        Ok(listener) => {
            println!("video-flow extension bridge on ws://0.0.0.0:{port}");
