#!/usr/bin/env bash
# Build the Email app and assemble an installable Space-App zip.
#
#   release/                 <- staged, flat install layout
#     email                  (release binary; manifest runtime.start = ./email)
#     senclaw-manifest.json
#     skills/                (email-reporting)
#     web_dist/              (built React UI — main.rs serves web_dist next to the binary)
#   email-app.zip            <- the artifact you install in SenClaw
#
# Usage: apps/email/scripts/pack.sh [--skip-build]
set -euo pipefail

APP_DIR="$(cd "$(dirname "$0")/.." && pwd)"
ROOT="$(cd "$APP_DIR/../.." && pwd)"
REL="$APP_DIR/release"
ZIP="$APP_DIR/email-app.zip"
BIN="$ROOT/target/release/email"

if [[ "${1:-}" != "--skip-build" ]]; then
  echo "==> building web UI"
  ( cd "$APP_DIR/web" && npm install --silent && npm run build )
  echo "==> building release binary"
  ( cd "$ROOT" && cargo build -p email --release )
fi

[[ -f "$BIN" ]] || { echo "missing $BIN — run without --skip-build"; exit 1; }
[[ -d "$APP_DIR/web/dist" ]] || { echo "missing web/dist — build the web UI first"; exit 1; }

echo "==> staging release/"
rm -rf "$REL" "$ZIP"
mkdir -p "$REL"
cp "$BIN" "$REL/email"
chmod +x "$REL/email"
cp "$APP_DIR/senclaw-manifest.json" "$REL/senclaw-manifest.json"
cp -R "$APP_DIR/skills" "$REL/skills"
cp -R "$APP_DIR/web/dist" "$REL/web_dist"

echo "==> zipping"
( cd "$REL" && zip -qr "$ZIP" senclaw-manifest.json email skills web_dist )
echo "packed: $ZIP"
