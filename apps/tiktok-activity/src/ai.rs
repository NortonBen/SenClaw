...
pub async fn generate_flow_from_catalog(
    ...
    let allowed: std::collections::HashSet<String> = catalog
        .iter()
        .map(|c| c.palette_id.trim().to_string())
        .filter(|s| !s.is_empty())
        .collect();
    ...
    let reply = bridge
        .llm(FLOWGEN_SYSTEM, &user, 8000, Duration::from_secs(180))
        .await?;
    let obj =
        extract_json_object(&reply.text).ok_or_else(|| anyhow!("LLM không trả JSON hợp lệ"))?;
    ...
    for (i, a) in actions.iter().enumerate() {
        ...
        if !allowed.contains(&pid) {
            return Err(anyhow!(
                "actions[{i}]: paletteId không có trong catalog: {pid:?}"
            ));
        }
...
pub async fn generate_profile_draft(
    ...
    let acct = account
        .map(|a| format!("account username={} id={}", a.username, a.id))
        .unwrap_or_default();
    let user = format!(
    ...
    match bridge
        .llm(
            "Bạn tạo fingerprint trình duyệt hợp lệ. Trả DUY NHẤT một JSON object.",
            &user,
            500,
        ...
        Ok(reply) => match extract_json_object(&reply.text) {
            Some(obj) => BrowserProfile {
                name: obj
                    ...
                    .map(|s| s.to_string())
                    .unwrap_or_else(|| heuristic().name),
                user_agent: obj
                    .get("userAgent")
                    .and_then(Value::as_str)
                    .filter(|s| !s.trim().is_empty())
                    .map(|s| s.to_string())
                    .unwrap_or_else(|| heuristic().user_agent),
                viewport_width: obj
                    .get("viewportWidth")
    ...
    for i in start..bytes.len() {
        ...
        match c {
            ...
            '}' => {
                depth -= 1;
                if depth == 0 {
                    return serde_json::from_str::<Value>(&s[start..=i])
                        .ok()
