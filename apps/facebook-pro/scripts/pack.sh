#!/usr/bin/env bash
# Build SenClaw Facebook Pro and assemble an installable Space-App zip.
#
#   release/                 <- staged, flat install layout
#     facebook-pro           (release binary; manifest runtime.start = ./facebook-pro)
#     senclaw-manifest.json
#     senclaw-hub.json
#     README.md
#     skills/  personas/
#     web_dist/              (built React UI — main.rs serves web_dist next to the binary)
#   facebook-pro-app.zip     <- the artifact you install in SenClaw
#
# Usage: apps/facebook-pro/scripts/pack.sh [--skip-build]
set -euo pipefail

APP_DIR="$(cd "$(dirname "$0")/.." && pwd)"
ROOT="$(cd "$APP_DIR/../.." && pwd)"
REL="$APP_DIR/release"
ZIP="$APP_DIR/facebook-pro-app.zip"
BIN="$ROOT/target/release/facebook-pro"

if [[ "${1:-}" != "--skip-build" ]]; then
  echo "==> building web UI (React + Ant Design)"
  ( cd "$APP_DIR/web" && npm install --silent && npm run build )
  echo "==> building release binary"
  ( cd "$ROOT" && cargo build -p facebook-pro --release )
fi
...
rm -rf "$REL" "$ZIP"
mkdir -p "$REL"
cp "$BIN" "$REL/facebook-pro"
chmod +x "$REL/facebook-pro"
