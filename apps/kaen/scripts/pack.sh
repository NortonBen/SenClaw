#!/usr/bin/env bash
# Build SenClaw Kaen and assemble an installable Space-App zip.
#
#   release/              <- staged, flat install layout
#     kaen                (release binary; manifest runtime.start = ./kaen)
#     senclaw-manifest.json
#     skills/             (kaen-study-coach)
#     personas/           (study-coach)
#     web_dist/           (built React UI — main.rs serves web_dist next to the binary)
#   kaen-app.zip          <- the artifact you install in SenClaw
#
# Usage: apps/kaen/scripts/pack.sh [--skip-build]
set -euo pipefail

APP_DIR="$(cd "$(dirname "$0")/.." && pwd)"
ROOT="$(cd "$APP_DIR/../.." && pwd)"
REL="$APP_DIR/release"
ZIP="$APP_DIR/kaen-app.zip"
BIN="$ROOT/target/release/kaen"

if [[ "${1:-}" != "--skip-build" ]]; then
  echo "==> building web UI"
  ( cd "$APP_DIR/web" && npm install --silent && npm run build )
  echo "==> building release binary"
  ( cd "$ROOT" && cargo build -p kaen --release )
fi
