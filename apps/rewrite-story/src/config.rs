...
/// App HTTP port — injected by the SenClaw daemon as PORT (manifest: 4470).
pub fn http_port() -> String {
    env_or("PORT", "4470")
}

pub fn app_id() -> String {
    env_or("SENCLAW_SPACE_APP_ID", "rewrite-story")
}

/// Base URL of the SenClaw daemon's UI server, used for the bridge.
...
/// home; `REWRITE_STORY_DATA_DIR` overrides it.
pub fn data_dir() -> PathBuf {
    if let Ok(d) = std::env::var("REWRITE_STORY_DATA_DIR") {
        if !d.trim().is_empty() {
