//! `search-mcp` — hand-rolled JSON-RPC MCP over HTTP + SSE.
//!
...
//! `search_query` instead of each re-implementing retrieval.
//!
//! P0 ships the retrieval half of the surface. `search_ask`, `search_deep`,
//! `search_report`, `search_verify`, corpus and monitor tools land in P1–P4.
...
pub fn tools_list() -> Vec<Value> {
    vec![
            ...
            "inputSchema": {
                ...
                "properties": {
                    ...
                    "sources": { "type": "array", "items": { "type": "string" },
                                 "description": "Giới hạn ở các nguồn này (id lấy từ search_sources). Bỏ trống = mọi nguồn đang bật." },
        ...
        json!({
            "name": "search_ask",
            "description": "Tìm kiếm RỒI rút ra các khẳng định nguyên tử, mỗi khẳng định gắn với bằng chứng cụ thể và được xếp hạng tin cậy bằng cách ĐẾM số nguồn độc lập (không phải do mô hình tự chấm). Khi các nguồn mâu thuẫn, kết quả trả về `disputed` kèm cả hai phía chứ KHÔNG tự chọn một bên. Chậm và tốn hơn search_query vì có gọi LLM — dùng khi cần câu trả lời có kiểm chứng, còn chỉ cần danh sách kết quả thì dùng search_query. LƯU Ý: điểm tin cậy đo độ chứng thực của nguồn, không đo tính đúng sai.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "query":   { "type": "string" },
        ...
        json!({
            ...
            "description": "Liệt kê các công cụ của một MCP bất kỳ (một Space App đã cài, hoặc một URL JSON-RPC). Dùng TRƯỚC search_source_add để biết tên công cụ và tên tham số thật, thay vì đoán.",
            "inputSchema": {
                "type": "object",
                "properties": {
        ...
        json!({
            "name": "search_source_remove",
            "description": "Gỡ một nguồn MCP do người dùng đăng ký. Không gỡ được nguồn có sẵn (web/knowledge/wiki) — hãy dùng search_source_config để tắt.",
            "inputSchema": {
                "type": "object",
        ...
        json!({
            ...
            "description": "Thêm một tài liệu dạng VĂN BẢN vào kho tài liệu của app (tự cắt đoạn và lập chỉ mục FTS tiếng Việt). Sau đó nội dung này tham gia mọi lần search_query như một nguồn. Muốn tải tệp PDF/DOCX thì dùng giao diện web hoặc POST /api/corpus.",
            "inputSchema": {
                "type": "object",
                "properties": {
...
pub async fn call_tool(state: &AppState, name: &str, args: &Value) -> Value {
    match name {
                ...
                Ok((raw_claims, raw_contradictions)) => {
                    ...
                    if let Some(id) = &run_id {
                        if let Err(e) = state.core.db.save_claims(id, &claims, &contradictions) {
                            eprintln!("[search] không lưu được claims: {e}");
                        }
                    }
        ...
        "search_source_remove" => {
            let Some(id) = arg_str(args, "source_id") else {
                return error_result("thiếu tham số `source_id`".into());
            };
            if crate::sources::mcp_source::RESERVED_IDS.contains(&id.as_str()) {
                return error_result(format!(
                    "`{id}` là nguồn có sẵn, không gỡ được — dùng search_source_config để tắt"
                ));
            }
        ...
        "search_corpus_list" => match state.core.db.list_documents() {
            Ok(docs) => json_result(json!({ "documents": docs })),
            Err(e) => error_result(e.to_string()),
        },

        "search_corpus_remove" => {
...
mod tests {
    ...
    async fn a_missing_required_argument_is_a_tool_error_not_a_panic() {
        let state = test_state();
        let out = call_tool(&state, "search_query", &json!({})).await;
