...
pub fn app_id() -> String {
    env_or("SENCLAW_SPACE_APP_ID", "search")
}
...
pub fn browser_agent_id() -> String {
    env_or("SENCLAW_AGENT_ID", "space-app-search")
}
...
pub fn data_dir() -> PathBuf {
    if let Ok(d) = std::env::var("SEARCH_DATA_DIR") {
        if !d.trim().is_empty() {
...
pub fn fanout_concurrency() -> usize {
    env_or("SEARCH_FANOUT_CONCURRENCY", "8")
        .parse()
        .unwrap_or(8)
}
...
pub fn source_timeout_ms() -> u64 {
    env_or("SEARCH_SOURCE_TIMEOUT_MS", "20000")
        .parse()
        .unwrap_or(20_000)
}
