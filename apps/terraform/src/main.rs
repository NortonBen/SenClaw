//! SenClaw Terraform Space App — quản lý dự án Terraform: workspace từ thư mục
//! local hoặc clone từ git (tự pull trước khi apply), đọc variables.tf render
//! form Apply + điền/lưu file .tfvars (vd prod.tfvars), console chạy
//! init/plan/apply/destroy stream lỗi trực tiếp, và tự cài Terraform CLI đa
//! nền tảng nếu máy chưa có. Dữ liệu 100% local (SQLite).
...
async fn main() {
    ...
    let port = std::env::var("PORT").unwrap_or_else(|_| "4770".to_string());
    ...
    let candidates = [
        std::path::PathBuf::from("apps/terraform/web/dist"),
    ...
    let listener = tokio::net::TcpListener::bind(format!("{host}:{port}"))
        .await
        .unwrap();
    println!("SenClaw Terraform running on http://{host}:{port}");
