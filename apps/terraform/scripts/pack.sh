#!/usr/bin/env bash
# Build SenClaw Terraform app and assemble an installable Space-App zip.
#
#   release/                 <- staged, flat install layout
#     terraform              (release binary; manifest runtime.start = ./terraform)
#     senclaw-manifest.json
#     skills/ personas/
#     web_dist/              (built React UI — main.rs serves web_dist next to the binary)
#   terraform-app.zip        <- the artifact you install in SenClaw
#
# Usage: apps/terraform/scripts/pack.sh [--skip-build]
set -euo pipefail

APP_DIR="$(cd "$(dirname "$0")/.." && pwd)"
ROOT="$(cd "$APP_DIR/../.." && pwd)"
REL="$APP_DIR/release"
ZIP="$APP_DIR/terraform-app.zip"
BIN="$ROOT/target/release/terraform"

if [[ "${1:-}" != "--skip-build" ]]; then
  echo "==> building web UI"
  ( cd "$APP_DIR/web" && npm install --silent && npm run build )
  echo "==> building release binary"
  ( cd "$ROOT" && cargo build -p terraform --release )
fi
...
rm -rf "$REL" "$ZIP"
mkdir -p "$REL"
cp "$BIN" "$REL/terraform"
chmod +x "$REL/terraform"
