//! SenClaw Warehouse Space App — quản lý kho hàng: danh mục sản phẩm, nhiều
//! kho, phiếu nhập / xuất / chuyển kho / điều chỉnh kiểm kê, thẻ kho, giá vốn
//! bình quân, báo cáo nhập-xuất-tồn, cảnh báo tồn tối thiểu + AI phân tích qua
//! bridge. Dữ liệu 100% local (SQLite) — app chỉ GHI SỔ kho, không kết nối
//! sàn/đơn vị vận chuyển nào.
...
async fn main() {
    ...
    let port = std::env::var("PORT").unwrap_or_else(|_| "4630".to_string());
    ...
    let candidates = [
        std::path::PathBuf::from("apps/warehouse/web/dist"),
    ...
    let listener = tokio::net::TcpListener::bind(format!("{host}:{port}"))
        .await
        .unwrap();
    println!("SenClaw Warehouse running on http://{host}:{port}");
