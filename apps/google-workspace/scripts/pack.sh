#!/usr/bin/env bash
# Build SenClaw Google Workspace and assemble an installable Space-App zip.
#
#   release/                    <- staged, flat install layout
#     google-workspace          (release binary; manifest runtime.start = ./google-workspace)
#     senclaw-manifest.json
#     skills/ personas/
#     web_dist/                 (built React UI — main.rs serves web_dist next to the binary)
#   google-workspace-app.zip    <- the artifact you install in SenClaw
#
# Usage: apps/google-workspace/scripts/pack.sh [--skip-build]
set -euo pipefail

APP_DIR="$(cd "$(dirname "$0")/.." && pwd)"
ROOT="$(cd "$APP_DIR/../.." && pwd)"
REL="$APP_DIR/release"
ZIP="$APP_DIR/google-workspace-app.zip"
BIN="$ROOT/target/release/google-workspace"

if [[ "${1:-}" != "--skip-build" ]]; then
  echo "==> building web UI"
  ( cd "$APP_DIR/web" && npm install --silent && npm run build )
  echo "==> building release binary"
  ( cd "$ROOT" && cargo build -p google-workspace --release )
fi
...
rm -rf "$REL" "$ZIP"
mkdir -p "$REL"
cp "$BIN" "$REL/google-workspace"
chmod +x "$REL/google-workspace"
