...
async fn main() {
    ...
    let port = std::env::var("PORT").unwrap_or_else(|_| "4490".to_string());
    ...
    let exe_dir = std::env::current_exe()
        ...
        .unwrap_or_default();
    let candidates = [
        std::path::PathBuf::from("apps/hub/web/dist"),
    ...
    let listener = tokio::net::TcpListener::bind(format!("0.0.0.0:{}", port))
        .await
        .unwrap();
    println!("SenClaw Device Hub running on http://0.0.0.0:{}", port);
