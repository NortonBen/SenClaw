...
pub struct AppState {
    /// Active workspace's wiki/index DB. Swapped per workspace so each folder
    /// keeps its own wiki (never shared). See `use_workspace`.
    pub db: std::sync::RwLock<Arc<Db>>,
...
impl AppState {
    /// The DB for the currently-open workspace.
    pub fn db(&self) -> Arc<Db> {
        self.db.read().unwrap().clone()
    }

    /// Switch to (open + migrate) the workspace-specific wiki DB for `root`.
    pub fn use_workspace(&self, root: &std::path::Path) -> Arc<Db> {
        let path = crate::deepwiki::db::wiki_db_path(root);
        // Reuse if already active (avoid reopening on every re-index).
        if self.db().get_meta("root").ok().flatten().as_deref() == Some(&*root.to_string_lossy()) {
            return self.db();
        }
        let db = Arc::new(Db::open(&path).expect("open workspace wiki db"));
        let _ = wiki::migrate(&db);
        let _ = db.set_meta("root", &root.to_string_lossy());
        *self.db.write().unwrap() = db.clone();
        db
    }
}
...
pub fn api_router() -> Router {
    // Placeholder DB until a workspace is opened (then swapped per-workspace).
    ...
    crate::deepwiki::watch::spawn_reindexer(state.clone(), watch_rx);
...
async fn index(
    ...
    // Switch to this workspace's own wiki DB before indexing (per-workspace scope).
...
async fn post_settings(
    ...
    crate::deepwiki::index::save_settings(&s.db(), &st).map_err(server)?;
...
async fn generate_wiki(
    State(s): State<Arc<AppState>>,
    body: Option<Json<GenWikiBody>>,
) -> Result<Json<Value>, ApiError> {
    let instructions = body.and_then(|b| b.0.instructions).unwrap_or_default();
    ...
    let extra = if instructions.is_empty() {
        String::new()
    } else {
        format!("\n\nYÊU CẦU CỦA NGƯỜI DÙNG (ưu tiên tuân theo): {instructions}")
    };
