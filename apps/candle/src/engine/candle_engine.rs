...
/// Supports CPU and Apple Silicon Metal (feature `metal`).
///
...
impl ModelArch {
    pub fn from_model_type(s: &str) -> anyhow::Result<Self> {
        match s {
...
pub fn detect_arch(config_str: &str) -> anyhow::Result<ModelArch> {
...
// `#[allow(unreachable_code)]` is needed because the `accelerate`
...
fn generate_with_cache(
    ...
    let max_kv_tokens = gen_opt
        .max_kv_tokens
        .unwrap_or(local_model_core::settings::DEFAULT_KV_WINDOW_TOKENS)
        .clamp(128, 262_144) as usize;
