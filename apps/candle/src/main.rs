//! Candle local-model Space App — macOS, Linux and Windows.
//!
//! Serves an OpenAI surface backed by the in-process Candle engine, plus the
...
async fn main() -> anyhow::Result<()> {
    ...
    let port = std::env::var("PORT").unwrap_or_else(|_| "4840".to_string());
    ...
    tracing::info!("SenClaw Candle running on http://{host}:{port}");
...
fn web_dir() -> ServeDir<tower_http::set_status::SetStatus<ServeFile>> {
    let exe_dir = std::env::current_exe()
        ...
        .unwrap_or_default();
    let candidates = [
        std::path::PathBuf::from("apps/candle/web"),
        exe_dir.join("web"),
