...
async fn main() {
    ...
    let port = std::env::var("PORT").unwrap_or_else(|_| "4350".to_string());
    ...
    let candidates = [
        std::path::PathBuf::from("apps/mindmap/web/dist"), // dev: cargo run from repo root
    ...
    let listener = tokio::net::TcpListener::bind(format!("0.0.0.0:{}", port))
        .await
        .unwrap();
    println!("SenClaw Mindmap running on http://0.0.0.0:{}", port);
