...
const TEXT_EXTS: &[&str] = &[
    "txt", "md", "markdown", "text", "csv", "tsv", "json", "log", "rtf", "org", "rst", "tex",
    "rs", "py", "js", "ts", "tsx", "jsx", "java", "kt", "c", "h", "cpp", "cc", "hpp", "go", "rb",
    "php", "swift", "sh", "html", "htm", "xml", "yaml", "yml", "toml", "ini", "conf", "sql",
];

const IMAGE_EXTS: &[&str] = &["png", "jpg", "jpeg", "webp", "gif", "bmp", "tif", "tiff", "heic"];
...
pub async fn extract_text(filename: &str, bytes: Vec<u8>) -> Result<(String, bool), String> {
    ...
    if ext == "pdf" {
        return Err(
            "File PDF chưa được hỗ trợ trực tiếp — hãy tải lên ảnh (chụp trang) hoặc văn bản.".into(),
        );
    }
    ...
    match std::str::from_utf8(&bytes) {
        ...
        _ => Err(format!("Không hỗ trợ định dạng file `.{ext}`. Hãy dùng ảnh hoặc file văn bản.")),
    }
}
...
async fn ocr_image(filename: &str, bytes: Vec<u8>) -> Result<String, String> {
    ...
    let form = reqwest::multipart::Form::new().part("image", part).text("language", "vi");
    ...
    if !status.is_success() {
        ...
        return Err(format!("OCR chưa sẵn sàng: {msg}. Cài model OCR trong SenClaw trước."));
    }
