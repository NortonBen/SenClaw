#!/usr/bin/env bash
# Build SenClaw Sandbox and assemble an installable Space-App zip.
#
#   release/              <- staged, flat install layout
#     sandbox             (release binary; manifest runtime.start = ./sandbox)
#     senclaw-manifest.json
#     skills/             (sandbox-runner)
#     personas/           (sandbox-runner)
#     web_dist/           (built React UI — main.rs serves web_dist next to the binary)
#   sandbox-app.zip       <- the artifact you install in SenClaw
#
# Usage: apps/sandbox/scripts/pack.sh [--skip-build]
set -euo pipefail

APP_DIR="$(cd "$(dirname "$0")/.." && pwd)"
ROOT="$(cd "$APP_DIR/../.." && pwd)"
REL="$APP_DIR/release"
ZIP="$APP_DIR/sandbox-app.zip"
BIN="$ROOT/target/release/sandbox"

if [[ "${1:-}" != "--skip-build" ]]; then
  echo "==> building web UI"
  ( cd "$APP_DIR/web" && npm install --silent && npm run build )
  echo "==> building release binary"
  ( cd "$ROOT" && cargo build -p sandbox --release )
fi
