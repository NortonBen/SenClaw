...
use crate::backend::{self, ExecSpec, Outcome, MAX_TIMEOUT_MS};
use crate::caps::{self, DirectKind};
use crate::code;
use crate::config;
use crate::db::{self, Db, NewSandbox, Run, Sandbox};
...
pub async fn create(db: &Db, req: CreateReq) -> Result<Sandbox> {
    let defaults = crate::settings::load(db);
...
pub async fn exec(
    ...
    let session = if sb.trace_enabled {
        match crate::trace::Session::begin(std::path::Path::new(&sb.workdir)) {
            Ok(s) => {
                ...
                for (k, v) in crate::trace::env(&home) {
...
mod tests {
    ...
    async fn open_mode_can_read_the_disk_and_strict_is_the_difference() {
        ...
        let mut r = req(Some("direct".into()));
        r.fs_mode = Some(crate::fsmode::FsMode::Open);
        let sb = create(&db, r).await.unwrap();
    ...
    async fn allowlist_mode_opens_exactly_the_configured_folder_and_no_more() {
        ...
        let mut r = req(Some("direct".into()));
        r.fs_mode = Some(crate::fsmode::FsMode::Allowlist);
        let sb = create(&db, r).await.unwrap();
