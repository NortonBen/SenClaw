...
use crate::{caps, code, config, files, mcp, monitor, mounts, pty, runner, settings};
...
async fn get_sandbox(State(s): State<AppState>, Path(id): Path<String>) -> ApiResult {
    ...
    // docker call per row.
    if sb.backend == "docker" && sb.status == "running" && !crate::backend::docker::is_running(&sb).await
    {
