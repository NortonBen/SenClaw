...
async fn main() {
    ...
    let port = std::env::var("PORT").unwrap_or_else(|_| "4370".to_string());
    ...
    // `web/dist` is checked LAST so running the binary from the repo root doesn't
    // pick up SenClaw's own `web/dist` (the static-dir collision gotcha).
    let candidates = [
        std::path::PathBuf::from("apps/skill-builder/web/dist"), // dev: cargo run from repo root
    ...
    let listener = tokio::net::TcpListener::bind(format!("0.0.0.0:{}", port))
        .await
        .unwrap();
    println!("SenClaw Skill Builder running on http://0.0.0.0:{}", port);
