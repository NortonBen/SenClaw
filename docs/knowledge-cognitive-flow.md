...
- **Decay ticker** (`decay_tick.rs`) — mỗi **300s** quét toàn bộ edges: áp decay, prune edge chết, thăng hạng LTP cho edge sống sót, ghi log vào `cog_decay_log` (UI: nút ⋯ → Decay log). Idempotent — sweep dở dang tự resume tick sau.
- **Maintenance sweep** (`maintenance.rs`, mặc định 24h/lần hoặc bấm tay từ UI):
  ...
  2. `merge_duplicate_entities` — gộp entity trùng tên chuẩn hoá, redirect edges về node canonical, cộng dồn mention count.
  3. `merge_alias_entities` — **vector alias merge (0 token LLM)**: quét top-300 entity theo mention_count, cặp nào cùng `type_name` và cosine similarity của embedding (đã trả tiền lúc ingest) ≥ 0.9 thì gộp vào node canonical (mention cao hơn thắng, redirect edges như merge tên). Bắt các alias khác chữ như "HN" ↔ "Hà Nội" mà merge tên không thấy.
